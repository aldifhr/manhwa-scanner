...
sb = get_supabase()
cut = (datetime.now(timezone.utc) - timedelta(hours=72)).isoformat()
rc = sb.table("recent_chapters").select("title_key, source, series_url, rating, genres").gte("updated_time", cut).execute()
rows = rc.data or []
