"""Core routers — health/dashboard/catalog/auth/dispatch."""
from app.api import dashboard as dashboard_api, catalog as catalog_api, auth as auth_api, dispatches as dispatches_api, observability as observability_api, system as system_api, rss as rss_api, health as health_api, error_logs as error_logs_api, analytics as analytics_api, audit_log as audit_log_api, cron_health as cron_health_api
from app.api.audit_stale import router as audit_stale_router
from app.api.enrich_missing import router as enrich_missing_router
from app.api.cron_status import router as _cron_status_router
from app.api.cron_health import router as _cron_health_router

def register_core(app):
    app.include_router(health_api.router, prefix="/api/v1")
    app.include_router(dashboard_api.router, prefix="/api/v1")
    app.include_router(catalog_api.router, prefix="/api/v1")
    app.include_router(auth_api.router, prefix="/api/v1")
    app.include_router(dispatches_api.router, prefix="/api/v1")
    app.include_router(observability_api.router, prefix="/api/v1")
    app.include_router(rss_api.router, prefix="/api/v1")
    app.include_router(_cron_status_router, prefix="/api/v1")
    app.include_router(_cron_health_router, prefix="/api/v1")
    app.include_router(system_api.router, prefix="/api/v1")
    app.include_router(error_logs_api.router, prefix="/api/v1")
    app.include_router(analytics_api.router, prefix="/api/v1")
    app.include_router(audit_log_api.router, prefix="/api/v1")
    app.include_router(audit_stale_router, prefix="/api/v1")
    app.include_router(enrich_missing_router, prefix="/api/v1")
