...
# Verify
print("\n=== Final verification ===")
for tk in ["the-maid-with-a-child", "i-may-be-a-beginner-but-i-m-actually-a-level-99-overpowered-player"]:
    cur.execute("SELECT chapter_num::int FROM recent_chapters WHERE title_key = %s ORDER BY chapter_num", (tk,))
    chapters = sorted([r["chapter_num"] for r in cur.fetchall()])
    expected = set(range(int(chapters[0]), int(chapters[-1]) + 1))
    actual = set(int(c) for c in chapters)
    gaps = sorted(expected - actual)
    print(f"  {tk}: {len(chapters)} ch, range {chapters[0]}-{chapters[-1]}, gaps={gaps if gaps else 'NONE'}")
