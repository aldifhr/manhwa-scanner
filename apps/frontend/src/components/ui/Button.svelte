<script lang="ts">
  ...
  const sizes: Record<string,string> = { default: "px-4 py-2 text-sm", sm: "px-3 py-1 text-xs", lg: "px-6 py-3" };
</script>
<button class={cn(base, variants[variant] ?? variants.default, sizes[size] ?? sizes.default, className)} {...rest}>
  {@render children?.()}
</button>
