...
function b64UrlDecode(input: string): string {
  ...
  try {
    const buf = (globalThis as { Buffer?: { from: (str: string, enc: string) => { toString: () => string } } }).Buffer;
...
export function verifyToken(token: string): boolean {
  // Require a well-formed 3-segment token (header.payload.signature) so
  // garbage/malformed cookies fail fast. We can't verify the HS256 signature
  // (no AUTH_SECRET in the FE), so expiry is still the only real check here —
  // the backend re-validates every proxied call and is the source of truth.
