        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-14 sm:h-16 gap-4 sm:gap-8">
            <Link href="/" className="flex items-center gap-2 shrink-0" onClick={() => setOpen(false)}>
              <span className="text-white font-bold text-lg tracking-tight">ManhwaScan</span>
            </Link>

            <div className="hidden md:flex items-center gap-1 ml-4">
              {NAV.map(({ href, label, icon }) => (
                <NavItem key={href} href={href} label={label} icon={icon} active={isNavActive(href, pathname)} onPrefetch={prefetch} />
              ))}
            </div>

            <div className="flex-1" />
            <NavbarStatus />
            <div className="hidden md:block w-px h-6 bg-white/10 mx-2" aria-hidden />
            <button onClick={logout} title="Logout" aria-label="Logout" className="hidden md:flex items-center justify-center p-2 ml-1 rounded-lg text-white/60 hover:text-red-400 hover:bg-red-500/10 transition-colors">
              <SignOut size={18} />
            </button>

            <button onClick={() => setOpen(true)} className="md:hidden inline-flex items-center justify-center w-10 h-10 rounded-lg bg-white/5 border border-white/10 text-white hover:bg-white/10 transition-colors" aria-label="Open menu" aria-expanded={open}>
