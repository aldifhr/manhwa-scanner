...
export async function GET(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${backendUrl()}/api/catalog/resolve?url=${encodeURIComponent(url)}`, {
    ...
    if (!res.ok) {
      ...
      return errorResponse(errObj?.error?.message || `Upstream ${res.status}`, res.status);
    }
