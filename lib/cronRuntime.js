...
export async function runCronJob({
  ...
  if (!whitelist.length) {
    ...
    await appendCronLog(redisClient, {
      tag: "info",
      code: "no_whitelist",
      type: "short_circuit",
      source: "cron",
  ...
  await saveSourceHealthMap(redisClient, nextSourceHealth, SOURCE_KEYS);
...
export async function readCronStatusWithHealth(redisClient = redis) {
  ...
  if (!data) return null;
  if (data.sourceHealth) return data;

  return {
    ...data,
    sourceHealth: await loadSourceHealthSnapshot(redisClient, SOURCE_KEYS),
  };
}
