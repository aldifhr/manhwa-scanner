...
async function validateChannel(
  ...
  guildId,
  {
    ...
    log: logFn = () => {},
    warn: warnFn = () => {},
    validateDiscordChannelFn: validateFn = validateDiscordChannel,
  } = {},
) {
  return validateFn({
    redis: rc,
    ...
    onInvalid: async (err) => {
      ...
      if (status === 404 || status === 403) {
        ...
        const fails = await rc.incr(failKey);
        await rc.expire(failKey, 86400); // 24h reset
        ...
        warnFn("Bot token invalid");
      } else {
        warnFn(`Validate ${guildId.slice(-4)}: ${err.message}`);
      }
    },
  });
}
...
export async function runCronJob({
  ...
  try {
    ...
    // Safety Check: Avoid partial sends near timeout
