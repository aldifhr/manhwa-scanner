...
import { scrapeMangaUpdatesWithMeta } from "./scraper.js";
import {
  createWhitelistMatcher,
  getChapterNumber,
} from "./domain/manga.js";
...
const CRON_INFO_LOG_THROTTLE_SEC = Number(process.env.CRON_INFO_LOG_THROTTLE_SEC || 1800);
...
export { buildPreferredIkiruTitles, buildPreferredSecondaryTitles, buildPreferredSecondaryUrls };
  ...
  return validateDiscordChannelFn({
    ...
    onInvalid: async (err) => {
      ...
      if (status === 404 || status === 403) {
        ...
        await deleteGuildChannelFn(guildId);
      } else if (status === 401) {
...
async function loadValidatedGuilds({
  ...
  const lastValidationAt = typeof lastValidationState === "string"
    ? lastValidationState
...
export async function runCronJob({
  ...
  const channelValidationConcurrencyResolved = resolveChannelValidationConcurrency(
    process.env.CHANNEL_VALIDATION_CONCURRENCY,
  );
  const channelValidationConcurrency = channelValidationConcurrencyResolved.value;
  const log = (...args) => DEBUG && logger.debug({ args }, "debug");
  const warn = (...args) => logger.warn({ args }, "warn");
  const start = Date.now();

  // Run health check once a day
  try {
    const lastCheck = await redisClient.get("health:last-check");
    const oneDayMs = 24 * 60 * 60 * 1000;
    if (!lastCheck || Date.now() - new Date(lastCheck).getTime() > oneDayMs) {
      logger.info("Running daily health check...");
      const brokenLinks = await performHealthCheck().catch((err) => {
        warn("Health check failed:", err.message);
        return null;
      });

      if (brokenLinks && brokenLinks.length > 0) {
        logger.info({ count: brokenLinks.length }, "Broken links detected, sending notifications...");
        const guildChannels = await getAllGuildChannelsFn();
        const msg = `⚠️ **Broken Link Detector**\nFound **${brokenLinks.length}** broken links in the whitelist. Use \`/health\` to see the details.`;
        ...
        for (const channelId of Object.values(guildChannels)) {
          await sendEmbed({ 
            ...
            description: msg,
            color: 0xff0000 
          }, channelId).catch(err => warn(`Failed to notify channel ${channelId}:`, err.message));
        }
      }
    }
  } catch (err) {
    warn("Failed to check health-check throttle:", err.message);
  }
  ...
  if (!whitelist.length) {
    ...
    await appendCronLogThrottled(redisClient, {
      tag: "info",
      code: "no_whitelist",
      type: "short_circuit",
      source: "cron",
  ...
  timingMetrics.channelValidationMs = roundTimingMs(Date.now() - validationStart);
  ...
  const { items: allResults, sourceStates } = await scrapeMangaUpdatesWithMetaFn(redisClient, {
    disabledSources,
  ...
  await saveSourceHealthMap(redisClient, nextSourceHealth, SOURCE_KEYS, sourceHealthMap);
  timingMetrics.sourceHealthWriteMs = roundTimingMs(Date.now() - sourceHealthWriteStart);
  ...
  const { sent, skipped, failed } = await dispatchChapters({
    ...
    onChannelError: async (err, channelId) => {
      ...
      if (!guildId) return;
      await deleteGuildChannelFn(guildId).catch(() => {});
      await redisClient
  ...
  logger.info(
    { duration, sent, skipped, failed, guilds: activeGuildCount, timingMetrics: finalTimingMetrics },
    "done",
  );
