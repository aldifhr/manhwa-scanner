  ...
  return validateDiscordChannelFn({
    ...
    onInvalid: async (err) => {
      ...
      if (status === 404 || status === 403) {
        ...
        if (fails >= 3) {
          warn(`DISCONNECTED (Persistent): guild ${guildId.slice(-4)} ch ${channelId.slice(-4)} (${status}) - Fail count: ${fails}. NO_AUTO_DELETE enabled.`);
          // System will keep the entry but continue to log warnings until fixed.
        } else {
          warn(`DISCONNECTED (Transient?): guild ${guildId.slice(-4)} ch ${channelId.slice(-4)} (${status}) - Fail count: ${fails}/3`);
        }
      } else if (status === 401) {
...
export async function runCronJob({
  ...
  const { sent, skipped, failed } = await dispatchChapters({
    ...
    onChannelError: async (err, channelId) => {
      ...
      if (!guildId) return;
      warn(`DISCONNECTED (Dispatch): guild ${guildId.slice(-4)} ch ${channelId.slice(-4)} (${status}) - skipping delete.`);
      // await deleteGuildChannelFn(guildId).catch(() => {});
      await redisClient.hset(CHANNELS_VALIDATION_KEY, {
        [channelId]: JSON.stringify({ valid: false, expiresAt: Date.now() + CHANNEL_VALIDATION_CACHE_SEC * 1000 })
      }).catch(() => {});
    },
  });
...
export async function readCronStatusWithHealth(redisClient = redis) {
  ...
  const recommendations = await redisClient.get("health:recommendations") || [];
