...
const agentOptions = {
  ...
  timeout: 25000, // Socket timeout
  freeSocketTimeout: 25000, // Free socket timeout
};
...
export async function requestWithRetry(requestFn, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    // Check if we already passed the deadline before the attempt
    if (deadline > 0 && startTime >= deadline) {
      logger.warn({ attempt, deadline }, "Request aborted: deadline reached before attempt");
      throw new Error("deadlineExceeded");
    }
    ...
    try {
      ...
      // Check if the delay + some safety margin (e.g. 1s) would exceed the deadline
      if (deadline > 0 && Date.now() + delayMs + 1000 >= deadline) {
        logger.warn({ attempt, delayMs, deadline }, "Request retry aborted: insufficient time before deadline");
        throw err; // Throw the original error or a specialized one
      }
