...
/** Discord message flag: EPHEMERAL (only visible to user) */
...
export const SCRAPER_LOOKBACK_HOURS = Number(process.env.SCRAPER_LOOKBACK_HOURS || 48);
export const ENQUEUED_EXPIRY_MS = Number(process.env.ENQUEUED_EXPIRY_MS || 15 * 60 * 1000); // 15 minutes

export const CHAPTER_TTL_SEC = 60 * 60 * 24 * 8; // 8 days memory
export const CHAPTER_PENDING_TTL_SEC = 60 * 10;
export const CROSS_SOURCE_DEDUPE_TTL_SEC = 60 * 60 * 24 * 4; // 4 days cross-source dedupe window
...
// ⚠️ WARNING: This constant CANNOT be used in `export const config = { maxDuration: ... }`
// Vercel bundler doesn't support imported constants in export statements.
// Use literal value `300` directly in api files instead.
...
/** Ikiru Scraper Configuration */
export const IKIRU_CONFIG = {
  REQUEST_TIMEOUT: resolvePositiveInt(process.env.IKIRU_REQUEST_TIMEOUT_MS, 12000),
  MAX_RETRIES: resolvePositiveInt(process.env.IKIRU_MAX_RETRIES, 1),
  RETRY_DELAY: 300,
  BATCH_SIZE: 15,
  TOTAL_TIMEOUT: resolvePositiveInt(process.env.IKIRU_TOTAL_TIMEOUT_MS, 17500),
  MAX_CHAPTERS_PER_RUN: resolvePositiveInt(process.env.IKIRU_MAX_CHAPTERS, 10),
  LATEST_MAX_PAGES: resolvePositiveInt(process.env.IKIRU_LATEST_MAX_PAGES, 6),
  CHAPTER_LIST_MAX_PAGES: resolvePositiveInt(process.env.IKIRU_CHAPTER_LIST_MAX_PAGES, 5),
};

/** Secondary (Shinigami) Scraper Configuration */
export const SECONDARY_CONFIG = {
  REQUEST_TIMEOUT: resolvePositiveInt(process.env.SECONDARY_REQUEST_TIMEOUT_MS, 9000),
  MAX_RETRIES: resolvePositiveInt(process.env.SECONDARY_MAX_RETRIES, 1),
  RETRY_DELAY: 500,
  BATCH_SIZE: 10,
  TOTAL_TIMEOUT: 17500,
  MAX_CHAPTERS_PER_RUN: 10,
  DIRECT_FALLBACK_MAX_URLS: resolvePositiveInt(process.env.SECONDARY_DIRECT_FALLBACK_MAX_URLS, 100),
  DETAIL_MAX_MANGA: resolvePositiveInt(process.env.SECONDARY_DETAIL_MAX_MANGA, 10),
  CHAPTER_LIST_MAX_PAGES: resolvePositiveInt(process.env.SECONDARY_CHAPTER_LIST_MAX_PAGES, 3),
};

/** Unified safe configuration for scrapers (Backward Compatibility) */
export const FASTCRON_SAFE_CONFIG = {
  REQUEST_TIMEOUT: SCRAPER_REQUEST_TIMEOUT_MS,
  TOTAL_TIMEOUT: 17500,
  MAX_RETRIES: 1,
  RETRY_DELAY: 500,
  MAX_CHAPTERS_PER_RUN: 5,
  MAX_CHAPTERS: 5,
  DIRECT_FALLBACK_MAX_URLS: 100,
};

// Aliases for backward compatibility removed (CHAPTER_TTL)
...
// Allowed domains for URL validation (whitelist API)
