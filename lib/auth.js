...
function buildThrottleSnapshot(count = 0, retryAfterSec = 0) {
  if (!Number.isFinite(count) || count <= 0 || !Number.isFinite(retryAfterSec) || retryAfterSec <= 0) {
    return {
      count: 0,
      limited: false,
      retryAfterSec: 0,
...
export async function readDashboardLoginThrottle(redis, req, nowMs = Date.now()) {
  ...
  if (!state || typeof state !== "object") return buildThrottleSnapshot(null, nowMs);
...
export async function registerDashboardLoginFailure(redis, req, nowMs = Date.now()) {
  ...
  const next = {
    count: Number.isFinite(currentCount) && expiresAt > nowMs ? currentCount + 1 : 1,
