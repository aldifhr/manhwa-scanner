...
function constantTimeEqual(a, b) {
  ...
  // Use SHA-256 hashing to prevent timing attacks on password length
  // This ensures both sides are always the same length (32 bytes)
  const leftHash = createHmac("sha256", "constant-time-eq").update(left).digest();
  const rightHash = createHmac("sha256", "constant-time-eq").update(right).digest();
...
function getClientAddress(req) {
  ...
  if (forwarded) {
    // Security: Use the rightmost IP in X-Forwarded-For to prevent spoofing
    // When behind a trusted proxy, the rightmost IP is the one added by the proxy closest to the server
    const ips = String(forwarded).split(",").map(ip => ip.trim()).filter(Boolean);
    if (ips.length > 0) {
      // Use the rightmost non-private IP or the rightmost IP if all are private
      const rightmostIp = ips[ips.length - 1];
      // Validate IP format to prevent injection
      if (isValidIpAddress(rightmostIp)) {
        return rightmostIp;
      }
    }
  }
...
export async function readDashboardLoginThrottle(
  ...
  const state = await redis.get(key).catch((err) => {
    logger.error({ err: err.message }, "Failed to read throttle state");
    return null;
  });
...
export async function registerDashboardLoginFailure(
  ...
  if (typeof redis.incr === "function" && typeof redis.ttl === "function") {
    const count = Number(
      await redis.incr(key).catch((err) => {
        logger.error({ err: err.message }, "Failed to increment throttle");
        return 1;
      }),
    );
    let ttlSec = Number(
      await redis.ttl(key).catch((err) => {
        logger.error({ err: err.message }, "Failed to get throttle TTL");
        return -2;
      }),
    );
    ...
    if (count === 1 || ttlSec <= 0) {
      await redis.expire(key, windowSec).catch((err) => {
        logger.error({ err: err.message }, "Failed to set throttle expiry");
      });
      ttlSec = windowSec;
    }
  ...
  const current = await redis.get(key).catch((err) => {
    logger.error({ err: err.message }, "Failed to read throttle count");
    return null;
  });
  ...
  await redis.set(key, next, { ex: ttlSec }).catch((err) => {
    logger.error({ err: err.message }, "Failed to write throttle state");
  });
...
export async function clearDashboardLoginThrottle(redis, req) {
  ...
  await redis.del(getDashboardLoginThrottleKey(req)).catch((err) => {
    logger.error({ err: err.message }, "Failed to clear throttle");
  });
}
...
export function createDashboardSessionToken() {
  ...
  // Generate cryptographically secure random bytes for unpredictability
  // Note: randomBytes is imported from node:crypto module
  const randomValue = randomBytes(16).toString("hex");
  const payload = JSON.stringify({
    exp: Date.now() + SESSION_TTL_SECONDS * 1000,
    rnd: randomValue, // Add randomness to prevent token prediction
  });
...
export function getSessionCookieHeader(req, token) {
  ...
  const cookie = `${SESSION_COOKIE_NAME}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${SESSION_TTL_SECONDS};${secure}`;
  logger.debug(
...
export function getClearSessionCookieHeader(req) {
  const secure = getSecureCookieFlag(req) ? " Secure;" : "";
  const cookie = `${SESSION_COOKIE_NAME}=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0;${secure}`;
