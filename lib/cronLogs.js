...
export function cronDailyStatsKey(rawTime = null) {
  return formatDateKey(rawTime); // Just use the date string as the Hash field
}
...
export async function readCronDailyStats(redis, days = 30, endDate = new Date()) {
  ...
  try {
    const toDelete = [];
    const fortyFiveDaysAgoStr = new Date(Date.now() - CRON_DAILY_STATS_TTL * 1000).toISOString().slice(0, 10);
...
export async function appendCronDailyStats(redis, entry = {}) {
  ...
  if (currentStr && typeof currentStr === 'string') {
    try { next = JSON.parse(currentStr); } catch { /* ignore */ }
  ...
  await redis.hset(CRON_DAILY_STATS_MASTER_KEY, { [key]: JSON.stringify(next) });
}
