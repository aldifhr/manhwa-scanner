...
function normalizeStatsRecord(date, raw = null) {
  ...
  return {
    ...
    // raw field removed to simplify output and match test expectations
  };
}
...
export async function readCronDailyStats(
  ...
  try {
    ...
    logger.warn({ err: err.message }, "Failed to read daily stats from Redis");
    masterData = {};
  }
  ...
  try {
    ...
    if (toDelete.length > 0) {
      redis.hdel(CRON_DAILY_STATS_MASTER_KEY, ...toDelete).catch((err) => {
        logger.debug({ err: err.message }, "Daily stats cleanup failed");
      });
    }
  } catch (e) {
    logger.debug({ err: e.message }, "Daily stats cleanup planning failed");
  }
