...
// ─── Search Ikiru ─────────────────────────────────────────────────────────────
// Endpoint: POST /wp-admin/admin-ajax.php?nonce=XXXX&action=search
// Param   : query=KEYWORD
// Nonce   : dari homepage, pattern admin-ajax.php?nonce=XXXX&action=search

export async function searchIkiru(keyword) {
  try {
    // ── Step 1: Ambil nonce dari homepage ─────────────────────────────────────
    let nonce = null;
    try {
      const pageRes = await withRetry(() => axios.get(SITE_URL, {
        headers: { "User-Agent": "Mozilla/5.0" },
        timeout: 8000,
      }));
      const match =
        pageRes.data.match(/admin-ajax\.php\?nonce=([a-f0-9]+)&(?:amp;)?action=search/) ||
        pageRes.data.match(/admin-ajax\.php\?nonce=([a-f0-9]+)/);
      nonce = match?.[1] || null;
      if (nonce) console.log("✅ Nonce:", nonce);
      else       console.warn("⚠️ Nonce not found");
    } catch (e) {
      console.warn("⚠️ Failed to fetch nonce:", e.message);
    }

    if (!nonce) {
      console.error("❌ Cannot search without nonce");
      return [];
    }
    ...
    if (!raw || raw === "0" || raw === "-1") {
      console.warn("⚠️ Empty response from search");
      return [];
    }

    return parseSearchHTML(raw, keyword);
  } catch (err) {
    console.error("❌ searchIkiru error:", err.message);
    return [];
  }
}

// ─── Parse HTML dari response search ─────────────────────────────────────────
// Struktur DOM yang dikonfirmasi:
// <a href="/manga/slug/">
...
function parseSearchHTML(html, keyword) {
  ...
  $("a").each((_, el) => {
    ...
    // Hanya ambil URL manga, bukan chapter
    if (!url.includes("/manga/") && !url.includes("/series/")) return;
    if (/\/chapter[-./]/i.test(url)) return;

    const fullUrl = url.startsWith("http") ? url : `${SITE_URL}${url.replace(/^\//, "")}`;
    if (results.find(r => r.url === fullUrl)) return;

    // Title: dari <h3> dalam <a>, atau alt img, atau aria-label
    const title =
      card.find("h3, h4, h2").first().text().trim() ||
      card.find("img").first().attr("alt") ||
      card.attr("aria-label") ||
      "";
    if (!title) return;

    // Cover: <img> langsung dalam <a>
    const cover =
      card.find("img").first().attr("src") ||
      card.find("img").first().attr("data-src") ||
      null;

    // Desc: <p> dalam <a>, bersihkan dari HTML tag sisa
    const rawDesc = card.find("p").first().text().trim();
    const desc    = rawDesc.length > 150
      ? rawDesc.substring(0, 147) + "..."
      : rawDesc;
    ...
    results.push({
      ...
      url:   fullUrl,
      cover: cover
        ? cover.startsWith("http") ? cover : `${SITE_URL}${cover.replace(/^\//, "")}`
        : null,
      desc,
    });
  });

  console.log(`✅ parseSearchHTML "${keyword}": ${results.length} results`);
  return results.slice(0, 10);
}