...
export async function searchIkiru(keyword) {
  const res = await withRetry(() => axios.get(
    `https://02.ikiru.wtf/advanced-search/?search_term=${encodeURIComponent(keyword)}`,
    {
      headers: { "User-Agent": "Mozilla/5.0" },
      timeout: 10000,
    }
  ...
  // tiap direct child div dari #search-results = 1 manga card
  $("#search-results > div").each((_, el) => {
    const card = $(el);

    // title & url dari link horizontal mode
    const titleLink = card.find("a[href*='/manga/']").first();
    const url       = titleLink.attr("href");

    // title dari h1 di dalam card
    const title = card.find("h1").first().text().trim()
                  || card.find("a.text-base").first().text().trim();

    // cover
    const cover = card.find("img.wp-post-image").first().attr("src");

    // status
    const status = card.find("span.bg-accent").first().text().trim();

    // chapter
    const chapter = card.find("span.text-sm").first().text().trim();

    // desc
    const desc = card.find("p.text-sm").first().text().trim();

    if (title && url && !seen.has(url)) {
      seen.add(url);
      results.push({ title, url, cover, status, chapter, desc });
    }
  });
