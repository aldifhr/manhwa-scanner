...
export async function scrapeMangaUpdatesWithMeta(redis = null, options = {}) {
  try {
    ...
    const sourceStates = {
      ikiru: { status: "pending", count: 0, error: null },
      shinigami_project: { status: "pending", count: 0, error: null },
      shinigami_mirror: { status: "pending", count: 0, error: null },
    };
    const allResults = [];
    const seen = new Set();
    const MAX_PAGES = 10;
    ...
    if (disabledSources.has("shinigami_project")) {
      ...
      try {
        shinigamiResults = await scrapeSecondarySourceUpdates(
          "shinigami_project",
          { throwOnError: true },
        );
        sourceStates.shinigami_project = {
    ...
    if (disabledSources.has("shinigami_mirror")) {
      ...
      try {
        mirrorResults = await scrapeSecondarySourceUpdates("shinigami_mirror", {
          throwOnError: true,
        });
        sourceStates.shinigami_mirror = {
          status: "ok",
    ...
    return {
      items: [],
      sourceStates: {
        ikiru: { status: "error", count: 0, error: err.message },
        shinigami_project: { status: "error", count: 0, error: err.message },
        shinigami_mirror: { status: "error", count: 0, error: err.message },
      },
    };
  }
}
