...
export async function searchIkiru(keyword) {
  // Step 1: ambil nonce
  const homePage   = await withRetry(() => axios.get(SITE_URL, {
    headers: { "User-Agent": "Mozilla/5.0" },
    timeout: 10000,
  }));
  const nonceMatch = homePage.data.match(/admin-ajax\.php\?nonce=([a-z0-9]+)/);
  if (!nonceMatch) throw new Error("Gagal ambil nonce dari Ikiru");
  const nonce = nonceMatch[1];

  // Step 2: POST search
  const res = await withRetry(() => axios.post(
    `https://02.ikiru.wtf/wp-admin/admin-ajax.php?nonce=${nonce}&action=search`,
  ...
  // Step 3: parse — title dari h3, desc dari p, cover dari img
  const $       = cheerio.load(res.data);
  const results = [];

  $("#searchResults a[href*='/manga/']").each((_, el) => {
    const card  = $(el);
    const url   = card.attr("href");
    const title = card.find("h3").first().text().trim();  // ← h3 saja
    const cover = card.find("img").first().attr("src");
    const desc  = card.find("p").first().text().trim();   // ← p saja

    if (title && url && !url.includes("advanced-search")) {
