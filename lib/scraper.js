...
const IKIRU_LATEST_MAX_PAGES = Number(
  process.env.IKIRU_LATEST_MAX_PAGES || 5,
);
const IKIRU_EMPTY_PAGE_BREAK_STREAK = Number(
...
const SECONDARY_CHAPTER_LIST_MAX_PAGES = Number(
...
export function shouldPrioritizeSecondaryTitle(title = "", preferredTitleKeys = null) {
  ...
  for (const preferred of preferredTitleKeys) {
    ...
    if (
      titleKey === preferred ||
      titleKey.includes(preferred) ||
      preferred.includes(titleKey)
    ) {
      return true;
    }
  }
...
async function fetchSecondaryRecentChapters(apiBase, mangaId) {
  const PAGE_SIZE = 24;
  const MAX_PAGES = Math.max(1, SECONDARY_CHAPTER_LIST_MAX_PAGES);
  const collected = [];
...
export async function scrapeMangaUpdatesWithMeta(redis = null, options = {}) {
  try {
    ...
    const allResults = [];
    const seen = new Set();
    const MAX_PAGES = Math.max(1, IKIRU_LATEST_MAX_PAGES);
    ...
    if (disabledSources.has("ikiru")) {
      ...
      for (let page = 1; page <= MAX_PAGES; page++) {
        ...
        if (emptyPageStreak >= IKIRU_EMPTY_PAGE_BREAK_STREAK) break;
      ...
      sourceStates.ikiru = {
        ...
        error:
          ikiruPageError && ikiruResults.length === 0
            ...
            : null,
        metrics: {
          pagesScanned: ikiruPagesScanned,
          stalePageStreak,
          emptyPageStreak,
          maxPages: MAX_PAGES,
        },
    ...
    const preferredSecondaryTitleKeysBySource = {
      shinigami_project: new Set(
        (Array.isArray(options?.preferredSecondaryTitles?.shinigami_project)
          ? options.preferredSecondaryTitles.shinigami_project
          : []
        ).map((title) => normalizeTitleKey(title)).filter(Boolean),
      ),
      shinigami_mirror: new Set(
        (Array.isArray(options?.preferredSecondaryTitles?.shinigami_mirror)
    ...
    const secondaryStateEntries = await Promise.all(
      secondarySources.map(async (source) => {
          ...
          return [
            source,
            {
              ...
              state: {
                ...
                count: 0,
                error: "cooldown active",
                metrics: defaultSecondaryMetrics,
              },
            },
          ];
        }
        ...
        try {
          const out = await scrapeSecondarySourceUpdates(source, {
            throwOnError: true,
            preferredTitleKeys: preferredSecondaryTitleKeysBySource[source],
          });
          return [
            source,
            {
    ...
    console.log(`[scrapeMangaUpdates] Total scraped: ${deduped.length} items`);
