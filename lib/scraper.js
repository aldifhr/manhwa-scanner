...
export async function scrapeMangaUpdates(redis = null) {
  ...
  if (redis) {
    try {
      ...
      if (cached) {
        ...
        const parsed = Array.isArray(cached) ? cached : JSON.parse(cached);
        return parsed;
      }
    } catch {}
  }
  ...
  try {
    ...
    // ✅ Selector yang sama dengan handleRecent yang sudah terbukti jalan
    ...
    if (redis) {
      ...
      console.log(`💾 Updates cached: ${results.length}`);
    }
