...
export async function searchIkiru(keyword) {
  ...
  $("#search-results .flex.rounded-lg").each((_, el) => {
    const card  = $(el);

    // ambil URL & title dari link utama
    const titleLink = card.find("a.text-base.font-medium").first();
    const title     = titleLink.text().trim();
    const url       = titleLink.attr("href");

    // cover dari img pertama di card
    const cover  = card.find("img.wp-post-image").first().attr("src");

    // status & chapter
    const status  = card.find("span.bg-accent").first().text().trim();
    const chapter = card.find("span.text-sm.text-gray-300").first().text().trim();

    // description
    const desc = card.find("p.text-sm.text-gray-300.line-clamp-3").first().text().trim();
