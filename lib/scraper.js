...
// ─── MAIN SCRAPER — scrapes /latest-update/ across multiple pages ─────────────
function parsePage($, seen) {
  ...
  cards.each((_, card) => {
    ...
    const mangaUrl = $vertical.find("a[href*='/manga/']").first().attr("href") || null;
    ...
    const status = $vertical.find("p.font-normal.text-xs")
    ...
    $chapterLinks.each((i, el) => {
      ...
      results.push({
        ...
        url,
        cover,
        mangaUrl: mangaUrl?.startsWith("http") ? mangaUrl : `${SITE_URL}${mangaUrl?.slice(1)}`,
        rating,
        status,
        updatedTime,
      });
...
export async function scrapeMangaUpdates(redis = null) {
  try {
    ...
    for (let page = 1; page <= MAX_PAGES; page++) {
      ...
      // Stop if all items on this page are older than 24h
