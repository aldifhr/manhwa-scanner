...
export async function sendTelegram(botToken, chatId, data) {
  try {
    ...
    message += `⭐ Rating: ${data.rating !== "N/A" ? `${data.rating}/10` : "No rating"}\n`;
    message += `📊 Status: ${STATUS_EMOJI[data.status] || "⚪"} ${data.status}\n`;
    
    if (data.updatedTime) {
      message += `🕐 Updated: ${formatTimeAgo(data.updatedTime)}\n`;
    }
    
    message += `\n[Read Chapter](${data.url})\n`;
    message += `${sourceEmoji} ${sourceText} • ikiru.wtf`;
