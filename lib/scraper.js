...
const SECONDARY_DETAIL_WINDOW_HOURS = Number(
  process.env.SECONDARY_DETAIL_WINDOW_HOURS || 2,
);
const SECONDARY_DETAIL_MAX_MANGA = Number(
  process.env.SECONDARY_DETAIL_MAX_MANGA || 10,
);
const SECONDARY_DETAIL_THROTTLE_MS = Number(
  process.env.SECONDARY_DETAIL_THROTTLE_MS || 200,
);

// â”€â”€â”€ COOKIE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/**
 * Ambil cookie â€” dari memory, Redis, atau login ulang.
...
async function getCookie(redis = null) {
  ...
  // 2. Sudah ada promise yang sedang berjalan â€” tunggu hasilnya
  ...
  _cookiePromise = (async () => {
    ...
    if (redis) {
      try {
        ...
        if (cached) {
          _cookie = cached;
          console.log("ðŸª Cookie loaded from Redis");
          return _cookie;
        }
      } catch (err) {
 ...
 * Untuk 429, hormati Retry-After jika tersedia.
 */
async function withRetry(fn, retries = 3, options = {}) {
...
function pickSecondaryDescription(row = {}) {
  const raw =
    row?.description ||
    row?.synopsis ||
    row?.summary ||
...
// â”€â”€â”€ EXPORTS HELPERS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function fetchSecondaryRecentChapters(apiBase, mangaId) {
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    const endpoint =
      ...
      `?page=${page}&page_size=${PAGE_SIZE}&sort_by=chapter_number&sort_order=desc`;
    ...
    for (const row of rows) {
      const updatedRaw =
        row?.release_date || row?.created_at || row?.updated_at || null;
      const parsedTime =
    ...
    if (!foundFreshOnThisPage || rows.length < PAGE_SIZE) break;
  }

  return collected;
}

async function scrapeSecondarySourceUpdates(
  source = "shinigami",
  { throwOnError = false } = {},
) {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const latestRaw = row?.latest_chapter_time || row?.updated_at || null;
      const latestParsed =
        parseIkiruDatetime(latestRaw) || parseLooseRelativeTime(latestRaw);
      ...
      if (shouldUseDetail) {
        ...
        try {
          ...
          chapterRows = await fetchSecondaryRecentChapters(apiBase, mangaId);
          detailCount += 1;
          ...
          if (status === 429) {
            ...
            metrics.detail429 += 1;
            console.warn(
              `[scrapeSecondarySourceUpdates] chapter detail 429, disable detail mode for this run (${normalized})`,
            );
          } else {
            console.warn(
              `[scrapeSecondarySourceUpdates] chapter list fallback for manga ${mangaId}:`,
              err.message,
            );
          }
      ...
      for (const chapterRow of chapterRows) {
        ...
        const chapterValue = chapterRow?.chapter_number;
        ...
        const updatedRaw = chapterRow?.created_at || row?.updated_at;
        const parsedTime =
          parseIkiruDatetime(updatedRaw) || parseLooseRelativeTime(updatedRaw);
        if (!parsedTime) continue;
...
export async function scrapeMangaUpdatesWithMeta(redis = null, options = {}) {
  try {
    ...
    if (disabledSources.has("ikiru")) {
      ...
      const ikiruResults = [];
      let ikiruPagesScanned = 0;
      const cookie = await getCookie(redis);
      console.log(
        cookie
          ? "[cookie] Scraping with cookie (realtime mode)"
          : "[cookie] Scraping without cookie (cached mode) - set IKIRU_EMAIL/PASSWORD for realtime",
      );

      for (let page = 1; page <= MAX_PAGES; page++) {
        ikiruPagesScanned = page;
        const url =
        ...
        console.log(`[ikiru] Page ${page}: ${results.length} items`);
      ...
      allResults.push(...ikiruResults);
      sourceStates.ikiru = {
        status: ikiruPageError && ikiruResults.length === 0 ? "error" : "ok",
        count: ikiruResults.length,
        error:
          ikiruPageError && ikiruResults.length === 0
            ? ikiruPageError.message
            : null,
        metrics: { pagesScanned: ikiruPagesScanned, stalePageStreak },
      };
    }

    let shinigamiResults = [];
    let mirrorResults = [];
    let shinigamiMetrics = null;
    let mirrorMetrics = null;

    if (disabledSources.has("shinigami_project")) {
      sourceStates.shinigami_project = {
        status: "skipped",
        count: 0,
        error: "cooldown active",
        metrics: {
      ...
      try {
        ...
        shinigamiResults = out.results;
        shinigamiMetrics = out.metrics;
        sourceStates.shinigami_project = {
          status: "ok",
          count: shinigamiResults.length,
          error: null,
          metrics: shinigamiMetrics,
        };
      } catch (err) {
        sourceStates.shinigami_project = {
          status: "error",
    ...
    if (disabledSources.has("shinigami_mirror")) {
      sourceStates.shinigami_mirror = {
        status: "skipped",
        count: 0,
        error: "cooldown active",
        metrics: {
      ...
      try {
        const out = await scrapeSecondarySourceUpdates("shinigami_mirror", {
          throwOnError: true,
        });
        mirrorResults = out.results;
        mirrorMetrics = out.metrics;
        sourceStates.shinigami_mirror = {
          status: "ok",
...
// â”€â”€â”€ ADVANCED SEARCH â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  for (const url of urls) {
    try {
      ...
      for (const pattern of NONCE_PATTERNS) {
        ...
        if (match) {
          console.log(`âœ… Nonce found at: ${url}`);
          nonceCache = {
            value: match[1],
...
function parseAdvancedSearchHTML(html) {
  ...
  $(".flex.rounded-lg.overflow-hidden").each((_, el) => {
    ...
    // Cover â€” bersihkan suffix ukuran WP
...
export async function searchIkiru(keyword, opts = {}, redis = null) {
  ...
  if (redis) {
    try {
      ...
      if (Array.isArray(cached)) {
        console.log(`âš¡ Cache hit "${keyword}": ${cached.length} results`);
        return cached;
      }
    } catch (err) {
  ...
  try {
    ...
    console.log(`âœ… Search "${keyword}": ${results.length} results`);
    return results;
  } catch (err) {
