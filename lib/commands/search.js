  ...
  const selectRow = {
    ...
    components: [{
      ...
      options:     slice.map((item, i) => {
        // value: slug truncated, fallback ke index kalau slug undefined/kosong
        const rawValue = (item.slug || item.url?.split("/manga/")[1]?.replace(/\/$/, "") || `result-${i}`);
        const value    = rawValue.substring(0, 100);
