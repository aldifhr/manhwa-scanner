...
export default function handleSearch(payload, options, res, redis = null) {
 ...
 res.json({ type: 5, data: { flags: 64 } }); // tambah flags: 64
...
export function buildSearchComponents(keyword, slice, page, totalPage) {
  const selectRow = {
    ...
    components: [{
      ...
      options:     slice.map((item, i) => {
        // value: "title|||url" — max 100 char, title di-truncate kalau perlu
        const urlPart   = item.url;
        const titlePart = item.title.length + urlPart.length + 3 <= 100
          ? item.title
          : item.title.substring(0, 100 - urlPart.length - 3);
        const value     = `${titlePart}|||${urlPart}`;

        // description wajib ada dan tidak boleh kosong
        const rawDesc  = item.chapter
          ? `${item.chapter}${item.status ? "  •  " + item.status : ""}`
          : (item.status ?? "manga");
        const description = rawDesc.substring(0, 100) || "manga";
