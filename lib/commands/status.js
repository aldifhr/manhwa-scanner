...
import { CACHE_TTL_LABEL, CRON_INTERVAL_LABEL, ONE_DAY_MS } from "../config.js";
...
async function handlePermissionManagement(payload, options) {
  if (!isOwner(payload)) {
    return editInteractionResponse(
      payload,
      "❌ Command manajemen izin hanya untuk owner bot.",
    );
  }
  ...
  const targetId = String(
    subOptions.find((o) => o.name === "user_id")?.value || "",
  ).trim();
  ...
  try {
    if (subcommand.name === "perm_add") {
      ...
      return editInteractionResponse(
        payload,
        `✅ Berhasil memberikan akses \`/add\` kepada <@${targetId}> (\`${targetId}\`).`,
      );
    }
    ...
    if (subcommand.name === "perm_remove") {
      ...
      return editInteractionResponse(
        payload,
        `✅ Berhasil mencabut akses \`/add\` dari <@${targetId}> (\`${targetId}\`).`,
      );
    }
    ...
    if (subcommand.name === "perm_list") {
      ...
      if (!users.length)
        return editInteractionResponse(
          payload,
          "📋 Belum ada user tambahan di whitelist.",
        );
      const list = users.map((id) => `• <@${id}> (\`${id}\`)`).join("\n");
      return editInteractionResponse(
        payload,
        `📋 **User dengan akses \`/add\` (Dinamis):**\n\n${list}`,
      );
    }
  } catch (err) {
