...
/**
 * Escape colons in custom_id components to prevent parsing issues
 * Discord custom_id has 100 char limit
 */
function escapeCustomId(str) {
  return String(str || "").replace(/:/g, "\\:");
}

function unescapeCustomId(str) {
  return String(str || "").replace(/\\:/g, ":");
}

function buildToggleButton(state, title, chapter) {
  const isRead = state === "read";
  // Sanitize title and chapter to prevent custom_id parsing issues
  const safeTitle = escapeCustomId(title.slice(0, 70));
  const safeChapter = escapeCustomId(chapter.slice(0, 20));
  return [
    {
      type: 1,
      components: [
        {
          ...
          custom_id: `${isRead ? "unread" : "read"}:${safeTitle}:${safeChapter}`,
          disabled: false,
        },
      ],
    },
...
function parseCustomId(customId) {
  if (!customId || typeof customId !== "string") {
    return { action: null, title: null, chapter: null };
  }
  ...
  const chapter = unescapeCustomId(parts.pop());
  const title = unescapeCustomId(parts.join(":"));
...
async function handleButtonClick(payload, res, redis, userId, options) {
  ...
  if (!action || !titleKey || !chapter) {
    return res.json({
      type: 4,
      data: { content: "Invalid button data format.", flags: 64 },
    });
  }

  // Security: Verify the clicking user is the same as the interaction user
  // This prevents other users from modifying someone else's progress
  const interactingUserId = payload.member?.user?.id ?? payload.user?.id;
  const messageUserId = payload.message?.interaction?.user?.id;

  // If there's a message user ID and it doesn't match the interacting user, reject
  if (messageUserId && messageUserId !== interactingUserId) {
    return res.json({
      type: 4,
      data: {
        content: "Kamu tidak bisa mengubah progress user lain.",
        flags: 64,
      },
    });
  }
