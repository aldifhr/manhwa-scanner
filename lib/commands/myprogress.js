...
export default async function handleMyProgress(payload, options, res, redis) {
  ...
  if (isButton) {
    ...
    if (!isMessageEphemeral) {
      // In public channels, don't update the common message button (type 7).
      // Instead, send an ephemeral confirmation message (type 4).
      return res.json({
        type: 4,
        data: {
          content: action === "read" 
            ? `📚 **${title}** (Chapter ${chapter}) sudah ditandai masuk ke progress baca kamu!` 
            : `Sip, penanda baca untuk **${title}** sudah dihapus.`,
          flags: 64,
        },
      });
    }
