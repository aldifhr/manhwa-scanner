  ...
  waitUntil(
    (async () => {
      try {
        ...
        let progressValues = await redis.hmget(progressDataKey, ...titleKeys);
        if (progressValues && !Array.isArray(progressValues) && typeof progressValues === "object") {
          progressValues = titleKeys.map(tk => progressValues[tk]);
        }
