...
async function buildStatusMessage(payload: { token: string; guild_id?: string }) {
  ...
  if (top5Raw.length > 0) {
    ...
    for (const item of whitelist) {
      ...
      if (tk) titleMap.set(tk, item.title);
    }

    for (let i = 0; i < top5Raw.length; i += 2) {
      const key = top5Raw[i] as string;
      const score = top5Raw[i + 1] as number;
  ...
  waitUntil(
    (async () => {
      try {
        ...
        logger.error({ err: errMessage }, "[handleStatus] Error");
        await editInteractionResponse(
          payload.token,
          `❌ Kesalahan: ${errMessage}`,
        );
      }
    })(),
  );
