...
export default function handleResync24h(payload, options, res) {
  ...
  waitUntil((async () => {
    ...
    try {
      ...
      const chapterKeys = matched.map((item) => `chapter:${normalizeUrl(item.url)}`);
      const sentFlags = chapterKeys.length ? await redis.mget(...chapterKeys) : [];
      const unsent = matched.filter((_, i) => !sentFlags[i]);
