...
export default async function handleList(payload, options) {
  try {
    const whitelist = await loadWhitelist();

    if (!whitelist.length) {
      return editInteractionResponseWithComponents(payload, "📋 Whitelist kosong!", []);
    }
    ...
    return editInteractionResponseWithComponents(payload, `❌ ${err.message}`, []);
  }
}