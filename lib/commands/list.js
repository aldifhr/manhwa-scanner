...
export default function handleList(payload, options, res) {
  res.json({ type: 5 });  // Defer dalam 3s
  ...
  waitUntil((async () => {
    try {
      ...
      const content = `📋 **Whitelisted Manga** (${whitelist.length} total)\n**Page ${page}/${totalPage}**\n\n${list}${totalPage > 1 ? `\n\n*Gunakan /list <page>*` : ""}`;

      // Truncate kalau terlalu panjang
      await editInteractionResponse(payload.token, content.substring(0, 2000));
    } catch (err) {
