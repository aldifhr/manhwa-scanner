...
export default async function handleList(payload, options, res, isComponent = false) {
  // Kalau slash command → kirim deferred reply di sini
  // Kalau dari button → res sudah dikirim di interactions.js, skip
  if (!isComponent) {
    res.json({ type: 5 });
  }
