...
function buildResyncGroupKey(item) {
  ...
  return url
    ? `${source}::${url}`
    : `${source}::${normalizeTitleKey(item?.title || "")}`;
}
...
async function handleQuickSync(payload) {
  try {
    const context = await loadMatchedDispatchContext({
      scrapeUpdates: (whitelist) =>
        scrapeMangaUpdates(
          redis,
          buildScrapeOptions(whitelist, { quick: true }),
        ),
    });

    if (context.status === "empty_whitelist")
      return editInteractionResponse(payload, "Whitelist kosong!");
    if (context.status === "no_channels")
      return editInteractionResponse(
        payload,
        "Belum ada notification channel.",
      );
...
async function handleDeepSync(payload, options) {
  if (!isOwner(payload))
    return editInteractionResponse(
      payload,
      "❌ Mode `deep` hanya untuk owner bot.",
    );

  const dryRun = Boolean(getOption(options, "dry_run"));
  const maxSend =
    Number(getOption(options, "max_send")) || RESYNC_DEFAULT_MAX_SEND;

  const lockVal = {
    by: payload.member?.user?.id ?? payload.user?.id,
    at: new Date().toISOString(),
  };
  if (
    !(await redis.set(LOCK_KEY, lockVal, { nx: true, ex: RESYNC_LOCK_TTL_SEC }))
  ) {
    return editInteractionResponse(
      payload,
      "Sync sedang berjalan. Tunggu beberapa menit.",
    );
  }
  ...
  try {
    const context = await loadMatchedDispatchContext({
      scrapeUpdates: (whitelist) =>
        scrapeMangaUpdates(redis, buildScrapeOptions(whitelist)),
    ...
    if (context.status === "no_matches")
      return editInteractionResponse(
        payload,
        "Tidak ada data rilis <24h untuk di-resync.",
      );
    if (context.status !== "ok")
      return editInteractionResponse(
        payload,
        `Sync dihentikan: ${context.status}`,
      );
...
export default function handleSync(payload, options, res) {
  ...
  const broadcast = Boolean(getOption(options, "broadcast"));
  const mode = getOption(options, "mode") || "quick";
