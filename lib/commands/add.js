...
const AUTOCOMPLETE_SEARCH_TIMEOUT_MS = 4000;
const AUTOCOMPLETE_PER_SOURCE_TIMEOUT_MS = 2500;
...
async function searchAddAutocompleteFast(query, redis = null) {
  const emptySource = { results: [], sourceUsed: "all", usedFallback: false };

  const [ikiruSettled, shngmSettled] = await Promise.allSettled([
    withTimeout(
      searchAddResultsFast(query, "ikiru", redis),
      AUTOCOMPLETE_PER_SOURCE_TIMEOUT_MS,
      emptySource,
    ),
    withTimeout(
  ...
  const ikiruResults =
    ikiruSettled.status === "fulfilled"
      ? (ikiruSettled.value?.results ?? [])
      : [];
  const shngmResults =
    shngmSettled.status === "fulfilled"
      ? (shngmSettled.value?.results ?? [])
      : [];

  return mergeSourceResults([
    { results: ikiruResults, usedFallback: false },
    { results: shngmResults, usedFallback: false },
  ]);
}
...
export async function buildAddAutocompleteChoices(options, redis = null) {
  ...
  const searchResult = await Promise.race([
    searchAddAutocompleteFast(query, redis),
    new Promise((resolve) =>
      setTimeout(() => resolve(timeoutResult), AUTOCOMPLETE_SEARCH_TIMEOUT_MS),
    ),
  ]);
  const results = Array.isArray(searchResult) ? searchResult : timeoutResult;
