...
async function enrichIkiruUpdatedTimes(results = [], redis = null) {
  ...
  await Promise.all(
    itemsToEnrich.map(item => concurrencyLimit(async () => {
...
export default async function handleAdd(payload, options, res, redis = null) {
  ...
  const urlOpt = /^https?:\/\//i.test(queryRaw) ? queryRaw : null;
  const query = urlOpt ? "" : queryRaw;

  // --- Jalur cepat: tambah langsung dari URL ---
  if (urlOpt) {
    res.json({ type: 5, data: { flags: 64 } });
    waitUntil(
      (async () => {
        try {
          const { title, source, error } = await resolveAddFromUrl(urlOpt);
          if (error) {
            return editInteractionResponse(payload, `❌ ${error}`);
          }
          const result = await addWhitelistEntry({ title, url: urlOpt, source });
          if (result.status === "exists") {
            return editInteractionResponse(payload, buildAddExistsMessage(result));
          }
          return editInteractionResponse(
            payload,
            buildAddSuccessMessage({ ...result, total: result.whitelist.length }),
          );
        } catch (err) {
  ...
  if (!query) {
    return res.json({
      ...
      data: { content: "Silakan masukkan judul manga atau URL langsung.", flags: 64 },
    });
  }
