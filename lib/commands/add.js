...
export async function buildAddAutocompleteChoices(options, redis = null) {
  const sourceRaw = getOption(options, "source");
  const source = !sourceRaw || sourceRaw === "all" ? "all" : normalizeSource(sourceRaw);
  const focused = options?.find((item) => item.focused);
  const query = String(focused?.value || "").trim();

  if (!query) return [];

  const { results } = await searchAddResults(query, source, redis);
  if (!results.length) return [];

  const top = results.slice(0, AUTOCOMPLETE_OPTION_LIMIT);
  const sessionId = createAddSessionId(source === "all" ? "all" : top[0]?.source || source);
...
export default function handleAdd(payload, options, res, redis = null) {
  const sourceRaw = String(getOption(options, "source") || "").trim();
  const source = !sourceRaw || sourceRaw === "all"
    ? "all"
