...
// Timeout wrapper for search calls
...
async function searchCombined(query, redis, options = {}) {
  ...
  // For autocomplete: 1.5s per source (need to respond under 3s)
  // For normal search: use default timeouts
  const ikiruTimeout = autocomplete ? 1500 : 8000;
  const shngmTimeout = autocomplete ? 1500 : 8000;

  const [ikiruResult, shngmResult] = await Promise.allSettled([
    withTimeout(searchIkiru(query, {}, redis), ikiruTimeout).catch(err => {
      logger.warn({ err: err.message }, "[searchCombined] Ikiru search failed");
      return [];
    }),
    withTimeout(searchShngm(query, "shinigami_project"), shngmTimeout).catch(err => {
...
export async function buildAddAutocomplete(options, redis) {
  ...
  try {
    // Autocomplete mode: fast 1.5s per source, total under 3s Discord limit
