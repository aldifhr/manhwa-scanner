...
function buildSearchOptions(
  ...
  const optionsSelect = top.map((item, i) => {
    const sourceLabel = item.source ? `[${item.source === "ikiru" ? "Ikiru" : "Shinigami"}] ` : "";
    const title = item.title.length > (AUTOCOMPLETE_LABEL_LIMIT - sourceLabel.length)
      ? `${item.title.substring(0, AUTOCOMPLETE_LABEL_LIMIT - sourceLabel.length - 3)}...`
      : item.title;
    return {
...
async function handleSearchAndDisplay(payload, query, redis) {
  try {
    // Search ALL sources (Ikiru + Shinigami)
    const { results, sourceUsed, usedFallback } = await searchAddResults(
      query,
      "all", // Changed from "ikiru" to "all" to search both sources
