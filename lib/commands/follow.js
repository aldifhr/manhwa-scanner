...
import { DISCORD_EPHEMERAL_FLAG, DISCORD_BUTTON_STYLE } from "../config.js";
...
export default async function handleFollow(
  ...
  if (
    ...
    if (subcommand === "list") {
      ...
      page = Number.isNaN(rawPage) || rawPage < 1 ? 1 : Math.min(rawPage, 100); // Max 100 pages
    } else {
      const rawPage = parseInt(options[0].value.split(":")[2], 10);
    ...
    waitUntil(
      (async () => {
        try {
          ...
          logger.error({ err: err.message }, "[handleFollow list] Error");
          return editInteractionResponse(
            payload,
