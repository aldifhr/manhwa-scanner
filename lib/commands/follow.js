...
export default async function handleFollow(payload, options, res, redisClient = redis) {
  const userId = payload.member?.user?.id ?? payload.user?.id;
  const subcommand = options?.[0]?.name;
  const subOptions = options?.[0]?.options || [];

  // Handle list subcommand or button pagination
  if (subcommand === "list" || (subcommand === "button" && options[0].value.startsWith("follow:list:"))) {
    let page = 1;
    if (subcommand === "list") {
      page = parseInt(subOptions.find(o => o.name === "page")?.value, 10) || 1;
    } else {
    ...
    waitUntil((async () => {
      try {
        const titleKeys = await redisClient.smembers(`user:follows:${userId}`);
        if (!titleKeys || titleKeys.length === 0) {
          return editInteractionResponse(payload, "Kamu belum mengikuti manga apa pun untuk notifikasi (ping).");
        }

        // Fetch display titles from whitelist:data
        let whitelistDataRaw = await redisClient.hmget("whitelist:data", ...titleKeys);
        ...
        const followData = titleKeys.map((tk, i) => {
        ...
        const lines = slice.map((item, i) => `${start + i + 1}. **${item.title}**`);
        ...
        const components = totalPage > 1 ? [
          {
            type: 1,
            components: [
              { type: 2, style: 1, label: "Sebelumnya", custom_id: `follow:list:${pageSafe - 1}`, disabled: pageSafe <= 1 },
              { type: 2, style: 2, label: `Hal ${pageSafe}`, custom_id: "noop", disabled: true },
              { type: 2, style: 1, label: "Berikutnya", custom_id: `follow:list:${pageSafe + 1}`, disabled: pageSafe >= totalPage },
            ]
          }
        ] : [];
        ...
        return editInteractionResponse(payload, `❌ Gagal memuat daftar: ${err.message}`);
      }
    })());
    return;
  }
  ...
  if (subcommand === "remove") {
    ...
    if (!query) return res.json({ type: 4, data: { content: "Masukkan judul manga.", flags: 64 } });
    ...
    waitUntil((async () => {
      try {
        ...
        return editInteractionResponse(payload, `✅ Berhasil unfollow **${query}**. Kamu tidak akan di-ping lagi untuk judul ini.`);
      } catch (err) {
  ...
  return res.json({ type: 4, data: { content: "Subcommand tidak valid.", flags: 64 } });
}
