...
export async function checkStaleMangas(redis, whitelist, payload) {
  ...
  try {
    ...
    if (timestamps && !Array.isArray(timestamps) && typeof timestamps === "object") {
  ...
  let warningsCache = [];
  try {
    warningsCache = await redis.hmget("manga:stale_warned", ...titleKeys);
    if (warningsCache && !Array.isArray(warningsCache) && typeof warningsCache === "object") {
  ...
  for (let i = 0; i < activeItems.length; i++) {
    ...
    if (warnedData) {
      const parsed = typeof warnedData === 'string' ? JSON.parse(warnedData) : warnedData;
  ...
  if (staleItems.length > 0) {
    ...
    await redis.hset("manga:stale_warned", warnsObj).catch(() => {});
  }

  // Cleanup expired entries periodically (fire and forget)
  redis.hgetall("manga:stale_warned").then((allFields) => {
    if (allFields && typeof allFields === 'object') {
      ...
      if (toDelete.length > 0) {
         redis.hdel("manga:stale_warned", ...toDelete).catch(()=>{});
      }
    }
  }).catch(()=>{ /* ignore */ });
