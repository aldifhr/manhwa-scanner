...
async function withWhitelistLock(redisClient, fn, { timeoutMs = 15000, ttlSec = 30 } = {}) {
  ...
  while (Date.now() - start < timeoutMs) {
    const acquired = await redisClient.set(lockKey, clientId, { nx: true, ex: ttlSec });
    if (acquired) break;
...
async function cleanupMangaData(redisClient, titleKey) {
  ...
  try {
    // 1. Get subscribers using the new Set-based structure
    const subscribers = await redisClient.smembers(`manga:subscribers:${titleKey}`);

    // 2. Optimized cleanup: Use SREM for each user's follows set
    const userCleanupTasks = subscribers.map(async (userId) => {
      // Clean up both old JSON and new Set storage
      await Promise.all([
        redisClient.srem(`users:follows:${userId}`, titleKey),
        (async () => {
          const oldJson = await redisClient.hget("users:follows", userId);
          if (oldJson) {
            const list = JSON.parse(oldJson);
            const filtered = list.filter((t) => t !== titleKey);
            if (filtered.length > 0) {
              await redisClient.hset("users:follows", { [userId]: JSON.stringify(filtered) });
            } else {
    ...
    await Promise.all([
      // Clean up manga-related keys
      redisClient.del(`manga:subscribers:${titleKey}`),
      redisClient.del(`manga:mutes:${titleKey}`),
      redisClient.del(`stale:warned:${titleKey}`),
      redisClient.hdel("manga:subscribers", titleKey), // Old JSON fallback
      redisClient.hdel("manga:mutes", titleKey),       // Old JSON fallback
