    ...
    if (exactMatches.length === 1) {
      ...
      if (partialMatches.length === 1) {
        ...
        const fuzzyMatches = list
          ...
          .filter((res) => res.score > 0.4) // Minimum threshold for fuzzy matches
...
export async function addWhitelistEntry({ title, url = null, source = "ikiru" }) {
  ...
  const initUpdateTask = redis.set(`manga:last_update:${titleKey}`, new Date().toISOString());
  ...
  if (existingIndex !== -1) {
    ...
    await Promise.all([persistWhitelistItems(whitelist), initUpdateTask]);
    return {
      status: "added",
      whitelist,
      source: effectiveSource,
