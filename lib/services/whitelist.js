...
export async function addWhitelistEntry({ title, url = null, source = "ikiru" }, { redisClient = redis, loadWhitelistFn = loadWhitelist, saveWhitelistFn = saveWhitelist } = {}) {
  ...
  const initUpdateTask = redisClient.hset("manga:last_updates", { [titleKey]: new Date().toISOString() });
...
async function cleanupMangaData(redisClient, titleKey) {
  ...
  try {
    ...
    console.error(`[cleanupMangaData] Error titleKey=${titleKey}:`, err.message);
  }
}
...
export async function removeWhitelistEntryIdentity(
  ...
  { loadWhitelistFn = loadWhitelist, saveWhitelistFn = saveWhitelist, redisClient = redis } = {}
) {
  const items = await loadWhitelistFn();
  const normalizedTitle = normalizeTitleKey(String(title || "").trim());

  const index = items.findIndex(item => normalizeTitleKey(item.title) === normalizedTitle);
  ...
  if (url) {
    ...
    const sourceIndex = item.sources?.findIndex(s => normalizeSourceUrl(s.url || "") === normUrl) ?? -1;
  ...
  if (source) {
    ...
    const sourceIndex = item.sources?.findIndex(s => normalizeSource(s.source || "") === normSource) ?? -1;
    if (sourceIndex === -1) return { status: "not_found", items };
...
export async function clearWhitelist() {
  ...
  const cleanupTasks = items.map(item => cleanupMangaData(redis, normalizeTitleKey(item.title)));
  
  await Promise.all([
    persistWhitelistItems([]),
    ...cleanupTasks
  ]);
...
export async function buildWhitelistListResponse(page = 1, pageSize = 10, { search = null, filter = null } = {}) {
  ...
  if (titleKeys.length > 0) {
    try { 
      let updateTimesRaw = await redis.hmget("manga:last_updates", ...titleKeys);
      if (updateTimesRaw && !Array.isArray(updateTimesRaw) && typeof updateTimesRaw === "object") {
