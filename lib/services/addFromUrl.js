...
const logger = getLogger({ scope: "addFromUrl" });
...
function fallbackIkiruTitleFromUrl(url = "") {
  try {
    const parsed = new URL(String(url || ""));
    const parts = parsed.pathname.split("/").filter(Boolean);
    const mangaIdx = parts.findIndex((part) => part.toLowerCase() === "manga");
    const slug = mangaIdx >= 0 ? parts[mangaIdx + 1] : null;
...
const IKIRU_BASE_LOWER = process.env.IKIRU_BASE_URL ? process.env.IKIRU_BASE_URL.toLowerCase() : null;
const SHIG_BASE_LOWER = process.env.SECONDARY_PUBLIC_BASE ? process.env.SECONDARY_PUBLIC_BASE.toLowerCase() : null;
...
export function detectSourceFromUrl(url) {
  ...
  if (IKIRU_BASE_LOWER && str.startsWith(IKIRU_BASE_LOWER)) return "ikiru";
  if (SHIG_BASE_LOWER && str.startsWith(SHIG_BASE_LOWER)) return "shinigami_project";
...
async function scrapeIkiruTitle(url) {
  const slugFallbackTitle = fallbackIkiruTitleFromUrl(url);
  try {
    const res = await httpGet(
      url,
      { headers: { "User-Agent": HTTP_USER_AGENT }, timeout: 8000 },
      { retries: 1, baseDelayMs: 400 },
    );
    const html = typeof res?.data === "string" ? res.data : null;
    if (!html) return slugFallbackTitle;
    ...
    logger.warn(
      { url, err: err.message },
      "Ikiru title scrape failed, falling back to slug title",
    );
  }
  return slugFallbackTitle;
}
...
async function scrapeShingmTitle(url) {
  ...
  if (uuidMatch && uuidMatch[1]) {
    try {
      ...
      logger.warn({ uuid: uuidMatch[1], err: err.message }, "API fallback failed");
    }
  }
  ...
  try {
    ...
    logger.warn({ url, err: err.message }, "HTML fallback failed");
  }
...
export async function resolveAddFromUrl(url) {
  ...
  if (source.startsWith("shinigami") && /\/chapter\b/i.test(cleanUrl)) {
    return {
      title: null,
      source,
      error: "URL Chapter Shinigami tidak diperbolehkan. Mohon masukkan URL halaman Series utama.",
    };
  }
