...
async function scrapeShingmTitle(url) {
  ...
  if (uuidMatch && uuidMatch[1]) {
    try {
      const apiRes = await httpGet(`https://api.shngm.io/v1/manga/detail/${uuidMatch[1]}`, {
      ...
      // Ignore API errors and fallback to HTML scraping
    }
  }
  ...
  for (const sel of selectors) {
    const title = $(sel).first().text().trim();
    if (title && title !== "undefined - Shinigami Scans" && title !== "Shinigami Scans") {
      return title;
    }
  }

  // Fallback for SvelteKit/Nuxt client-rendered pages
  ...
  if (titleMatch && titleMatch.length > 0) {
    for (const match of titleMatch) {
      ...
      if (
        extracted &&
        extracted.length > 2 &&
        extracted !== "Shinigami Scans" &&
        extracted !== "undefined - Shinigami Scans" &&
        extracted !== "Home"
      ) {
        return extracted;
      }
  ...
  // Fallback regex for standard title tag if cheerio misses it
