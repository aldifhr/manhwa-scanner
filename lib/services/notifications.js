...
export async function getMangaSubscribers(title) {
  ...
  try {
    // 1. Get explicit subscribers from hash
    const explicitSubscribers = await getHashJsonArray(SUBSCRIBERS_HASH, titleKey);

    // 2. Get users with "all" mode enabled
    const allModeUsers = await redis.smembers(ALL_MODE_SET);
