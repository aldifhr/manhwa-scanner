...
export async function saveSourceHealthMap(
  ...
  const current = currentMap || await loadSourceHealthMap(redis, sourceKeys);
  const writes = sourceKeys
    .filter((source) => {
      const nextValue = comparableSourceHealth(source, map[source]);
      const currentValue = comparableSourceHealth(source, current[source]);
