...
import { loadWhitelist, redis, SOURCES_HEALTH_KEY, SOURCE_KEYS } from "../redis.js";
export { SOURCE_KEYS };
...
export async function checkSingleLink(url) {
  const targetUrl = normalizeSourceUrl(url);
  const headers = {
    "User-Agent":
  ...
  try {
    ...
    return { url: targetUrl, status: res.status, ok: true };
  } catch {
    try {
      ...
      return { url: targetUrl, status: res.status, ok: isOk };
    } catch (e) {
      return {
        url: targetUrl,
...
export async function updateLinkStats(url, ok, status) {
  ...
  const hash = linkStatsHash(normalized);
  ...
  await redis.hset(HEALTH_STATS_KEY, { [hash]: JSON.stringify(failureStats) });
...
export async function performFullHealthCheck() {
  ...
  const recommendations = results
    .filter((r) => {
      ...
      // If it has NEVER succeeded, don't mark as stale unless it has multiple failures.
      // This prevents new links from being recommended for deletion on their first failure.
      if (!s.lastSuccessAt) {
        return s.consecutiveFailures >= 3;
      }

      const hoursSinceSuccess = (Date.now() - new Date(s.lastSuccessAt).getTime()) / 3600000;
  ...
  // Cleanup expired items from hash using hscan for scalability
  try {
    let cursor = "0";
    const batchSize = 100;
    const nowMs = Date.now();
    ...
    do {
      const results = await redis.hscan(HEALTH_STATS_KEY, cursor, {
        count: batchSize,
      });
    ...
    } while (cursor !== "0" && toDelete.length < 100);

    if (toDelete.length > 0) {
      await redis.hdel(HEALTH_STATS_KEY, ...toDelete).catch((delErr) => {
        logger.warn({ err: delErr.message }, "Failed to delete expired health stats");
      });
...
export function applySourceOutcome(
  ...
  if (outcomeStatus === "circuit_break") {
    // Specifically preserve "degraded" status and cooldown if we skipped the run
