...
export async function prepareDispatchQueue(redis, matched = [], maxItems = Infinity) {
  ...
  const existingFlags = validChapterMeta.length
    ...
    : [];
  const unsentMeta = validChapterMeta.filter((_, i) => !existingFlags[i]);
...
export async function dispatchChapters({
  ...
  const queueState = await prepareDispatchQueue(redis, matched, maxItems);
  ...
  const itemResults = await Promise.all(
    queueState.queuedMeta.map((entry) =>
      limit(async () => {
