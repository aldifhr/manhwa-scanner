import { normalizeTitleKey, compactTitleKey, normalizeChapterIdentity, createWhitelistMatcher } from "../domain.js";
import { normalizeSourceUrl } from "../domain.js";
import { getMangaSubscribers } from "./notifications.js";
import { getAllGuildChannels, loadWhitelist, redis as defaultRedis } from "../redis.js";
import {
  LOGS_API_CACHE_KEY,
...
function parseClaimState(value) {
  ...
  if (typeof value === "string") {
    try {
      ...
      return { status: parsed.status, claimedAt: parsed.claimedAt || null, sentAt: parsed.sentAt || null, expiresAt: parsed.expiresAt };
    } catch {
...
async function claimPendingChapter(redis, key, nowIso, pendingClaimTtl, pendingStaleMs) {
  ...
  const claimed = await redis.hsetnx(DISPATCH_HISTORY_KEY, key, JSON.stringify(claimPayload));
  ...
  await redis.hset(DISPATCH_HISTORY_KEY, { [key]: JSON.stringify(claimPayload) });
  return true;
}
...
export async function prepareDispatchQueue(
  ...
  let existingFlagsJson = validChapterMeta.length
    ? await redis.hmget(DISPATCH_HISTORY_KEY, ...validChapterMeta.map((entry) => entry.key))
    : [];
  if (existingFlagsJson && typeof existingFlagsJson === 'object' && !Array.isArray(existingFlagsJson)) {
    existingFlagsJson = validChapterMeta.map(e => existingFlagsJson[e.key]);
  }
  const existingFlags = (existingFlagsJson || []).map(j => typeof j === 'string' ? JSON.parse(j) : j);
  ...
  let duplicateValuesJson = duplicateKeys.length
    ...
    : [];
  if (duplicateValuesJson && typeof duplicateValuesJson === 'object' && !Array.isArray(duplicateValuesJson)) {
    duplicateValuesJson = duplicateKeys.map(k => duplicateValuesJson[k]);
  }
  const duplicateValues = (duplicateValuesJson || []).map(j => typeof j === 'string' ? JSON.parse(j) : j);
...
export async function dispatchChapters({
  ...
  for (const [index, entry] of queueState.queuedMeta.entries()) {
    ...
    const subscribers = await getMangaSubscribers(item.title).catch(() => []);
    const mentionChunks = [];
    const chunkSize = 50;
    ...
    if (sendResult.success > 0 && mentionChunks.length > 0) {
      ...
      for (const mChunk of mentionChunks) {
        await Promise.all(channelIds.map(channelId => 
          sendDiscordText(channelId, mChunk)
        )).catch(err => warn(`Failed extra pings: ${err.message}`));
      }
    }
    ...
    if (!success) {
      await Promise.all([
        ...
        duplicateKey ? redis.hdel(DISPATCH_HISTORY_KEY, duplicateKey) : Promise.resolve(0),
      ]);
  ...
  try {
    ...
    if (allFields && typeof allFields === 'object') {
      ...
      for (const [k, statStr] of Object.entries(allFields)) {
        try {
          const stat = typeof statStr === 'string' ? JSON.parse(statStr) : statStr;
