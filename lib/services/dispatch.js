 ...
 */
async function batchMarkChaptersSent(redisClient, items) {
  if (!items?.length) return;
  ...
  await withBatchLock(redisClient, "mark-sent", async () => {
    await execPipeline(redisClient, (pipeline) => {
      for (const { key, nowIso } of items) {
        pipeline.hset(DISPATCH_HISTORY_KEY, {
          [key]: JSON.stringify({ status: CLAIM_STATUS_SENT, sentAt: nowIso }),
        });
      }
    });
  });
...
function buildSuccessWriteTasks({
  ...
  const writeTasks = [
    redisClient.hset(DISPATCH_HISTORY_KEY, {
      [key]: JSON.stringify({
    ...
    redisClient.lpush("recent:chapters", {
      title: item.title,
...
async function cleanupRecentChapters(redisClient) {
  await Promise.all([
    redisClient.ltrim("recent:chapters", 0, 99),
    redisClient.expire("recent:chapters", RECENT_LIST_TTL_SEC),
  ]);
}
...
// Optimized cleanup using Redis scan instead of hgetall
async function scanAndCleanupExpired(redisClient, nowMs) {
  const toDelete = [];
  let cursor = 0;

  do {
    const result = await redisClient.hscan(DISPATCH_HISTORY_KEY, cursor, {
      count: 100,
    });
    cursor = result.cursor ?? 0;

    if (!result.fieldValues || result.fieldValues.length === 0) {
      continue;
    }

    for (let i = 0; i < result.fieldValues.length; i += 2) {
      const [key, statStr] = [result.fieldValues[i], result.fieldValues[i + 1]];
      const stat = safeJsonParse(statStr, statStr);

      if (stat?.expiresAt && stat.expiresAt < nowMs) {
        toDelete.push(key);
      }
    }
  } while (cursor !== 0);
