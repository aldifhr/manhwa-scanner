...
async function releaseBatchLock(redisClient, operation, token) {
  const current = await redisClient.get(`${BATCH_LOCK_KEY}:${operation}`);
  if (current === token) {
    await redisClient.del(`${BATCH_LOCK_KEY}:${operation}`);
  }
}
...
function buildSuccessWriteTasks({
  ...
  const writeTasks = [
    ...
    redisClient.hset("manga:last_updates", { [titleKey]: nowIso }),
    (async () => {
...
// Cleanup old recent chapters - hash-based with field-level TTL
async function cleanupRecentChapters(redisClient) {
  ...
  try {
    ...
    do {
      const result = await redisClient.hscan("recent:chapters", cursor, {
        count: 100,
      });
      ...
      for (let i = 0; i < fieldValues.length; i += 2) {
        ...
        try {
          const parsed = JSON.parse(value);
          // Delete if expired or no expiresAt field
    ...
    const currentCount = await redisClient.hlen("recent:chapters");
    if (currentCount > maxEntries) {
      const allData = await redisClient.hgetall("recent:chapters");
      const entries = Object.entries(allData)
        .map(([k, v]) => {
          try {
            return { key: k, data: JSON.parse(v) };
          } catch {
    ...
    console.warn("[cleanupRecentChapters] Error:", err.message);
  }
}
...
// Cleanup using simple hgetall (hscan not available in Upstash)
export async function scanAndCleanupExpired(redisClient, nowMs) {
  try {
    // Use hgetall to get all entries (simpler, works with Upstash)
    const allEntries = await redisClient.hgetall(DISPATCH_HISTORY_KEY);
    ...
    const toDelete = [];
    for (const [key, rawValue] of Object.entries(allEntries)) {
      // Handle both string (needs parsing) and object (already parsed)
      const value = typeof rawValue === "string" ? safeJsonParse(rawValue) : rawValue;
    ...
    logger.warn({ err }, "Error scanning dispatch hash");
    return [];
  }
}
  ...
  Promise.resolve()
    .then(async () => {
      try {
        ...
        logger.warn({ err }, "Error cleaning up dispatch hash");
      }
    })
    .catch(() => {});
}
