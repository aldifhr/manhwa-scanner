...
function normalizeChapterIdentity(chapter = "") {
...
export async function prepareDispatchQueue(
  ...
  const existingFlags = validChapterMeta.length
    ...
    : [];
  const duplicateFlags = validChapterMeta.length
    ? await Promise.all(
      validChapterMeta.map((entry) =>
        entry.duplicateKey ? redis.get(entry.duplicateKey) : Promise.resolve(null),
      ),
    )
    : [];
  ...
  const claimableMeta = validChapterMeta.filter(
    (_, i) =>
      ...
      !isBlockingClaim(duplicateFlags[i], pendingStaleMs, nowMs),
  );
