...
export async function getCachedOrFetch(
  ...
  if (redis) {
    try {
      ...
      console.warn(`[${context}] Redis get failed:`, err.message);
    }
  }
  ...
  try {
    ...
    if (redis && fresh !== null && fresh !== undefined) {
      try {
        ...
        console.warn(`[${context}] Redis set failed:`, cacheErr.message);
      }
    }

    return fresh;
  } catch (fetchErr) {
    console.error(`[${context}] Fetch failed:`, fetchErr.message);
    throw fetchErr;
  }
}
...
export function logWarnError(context, err, message = "") {
  ...
  console.warn(`${prefix}${msg} ${err?.message || err}`);
}
