...
export async function batchGetLastScrapeChecks(titleKeys) {
  ...
  const byShard = groupByShard(titleKeys);

  // Fall back to individual operations if pipeline not available
  if (typeof redis.pipeline !== "function") {
    return Promise.all(
      titleKeys.map((key) => redis.hget(getLastCheckHashKey(key), key)),
    );
  }

  const shardKeys = Array.from(byShard.entries()).map(([hashKey, fields]) => ({
    hashKey,
    fields,
  }));

  const pipeline = redis.pipeline();
  shardKeys.forEach(({ hashKey, fields }) =>
    pipeline.hmget(hashKey, ...fields),
  );

  const results = await pipeline.exec();
  return flattenResults(results, shardKeys, titleKeys);
}
...
export async function batchSetLastScrapeChecks(
  ...
  const byShard = groupByShard(titleKeys);
  const shardEntries = Array.from(byShard.entries());
  ...
  const pipeline = redis.pipeline();
  shardEntries.forEach(([hashKey, keys]) => {
    pipeline.hset(hashKey, buildHsetFields(keys, ts));
    if (hasTtlSupport) {
      addHexpireCommands(
        pipeline,
        hashKey,
...
const logWhitelistLoad = (indexLen, dataType, isArray, finalLen) => {
  console.log(
    `[loadWhitelist] Index=${indexLen} HMGET_Type=${dataType} Array=${isArray} Final=${finalLen}`,
  );
};
...
const buildWhitelistMaps = (normalized) => {
  ...
  normalized.forEach((item, i) => {
    ...
    if (!tk) return;
    dataMap[tk] = item;
    indexTasks.push(redis.zadd(WHITELIST_INDEX_KEY, { score: i, member: tk }));
  });

  return { dataMap, indexTasks };
};
...
export async function saveWhitelist(list) {
  ...
  const { dataMap, indexTasks } = buildWhitelistMaps(normalized);

  try {
    await Promise.all([
      redis.hset(WHITELIST_DATA_KEY, dataMap),
      redis.del(WHITELIST_INDEX_KEY).then(() => Promise.all(indexTasks)),
      redis.del(WHITELIST_KEY_LEGACY),
    ]);
  } catch (err) {
