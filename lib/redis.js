...
export async function setNotificationChannel(guildId, channelId) {
  try {
    // ✅ Wrap in JSON.stringify to force Upstash to store as string, not number
    // Prevents precision loss on large Discord snowflake IDs
    await redis.set(`channel:${guildId}`, JSON.stringify(String(channelId)));
  } catch (err) {
...
export async function getAllGuildChannels() {
  try {
    let cursor = 0;
    const keys = []; // ✅ deklarasi di luar loop

    do {
      // ✅ rename destructured variable jadi `batchKeys` agar tidak shadow `keys`
      const [nextCursor, batchKeys] = await redis.scan(cursor, {
        match: "channel:*",
        count: 100,
      });
      keys.push(...batchKeys);
      cursor = Number(nextCursor); // ✅ Upstash returns cursor as string, cast to number
    } while (cursor !== 0);
