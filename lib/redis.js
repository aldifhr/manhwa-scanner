...
export async function cleanupOldAlphabetShards() {
  ...
  if (deleted > 0) {
    console.log(`[cleanup] Deleted ${deleted} old alphabet-based shards`);
  }
...
export async function saveWhitelist(list) {
  ...
  try {
    ...
    console.log(`[saveWhitelist] Saved ${normalized.length} items`);
  } catch (err) {
    console.error("[saveWhitelist] Redis pipeline error:", err);
    // Attempt recovery - check if index needs rebuild
    const indexCount = await redis.zcard(WHITELIST_INDEX_KEY).catch(() => 0);
    if (indexCount === 0) {
      console.error(
        "[saveWhitelist] Index is empty after failed save! Attempting recovery...",
      );
      ...
      try {
        ...
        console.log(`[saveWhitelist] Recovered ${keys.length} index entries`);
      } catch (recoveryErr) {
        console.error("[saveWhitelist] Recovery failed:", recoveryErr);
      }
    }
    throw err;
  }
...
export async function readObjectCache(client, key) {
  try {
    ...
    console.error(`[redis] Failed to read cache ${key}:`, err.message);
    return null;
  }
}
