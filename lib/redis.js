    ...
    if (!seenTitles.has(key)) {
      ...
      result.push({
        ...
        sources: Array.isArray(item.sources) ? item.sources.map(s => ({
          ...
          mark: s.mark ? String(s.mark).trim() : null
        })) : []
      });
      continue;
    }

    // Merge sources if title already exists (shouldn't happen often if already migrated)
    const idx = seenTitles.get(key);
    const existing = result[idx];
    if (Array.isArray(item.sources)) {
      item.sources.forEach(s => {
        const hasSource = existing.sources.some(es => 
          es.source === normalizeSource(s.source) && 
          (!s.url || es.url === String(s.url).trim())
        );
        if (!hasSource) {
