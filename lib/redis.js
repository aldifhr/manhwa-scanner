...
function createMockRedisClient() {
  const mockOperation = (operation) => (...args) => {
    logger.warn({ operation, args: args.slice(0, 2) }, `Mock Redis: ${operation} called`);
    return Promise.resolve(null);
  };

  return {
    get: mockOperation("get"),
    set: mockOperation("set"),
    del: mockOperation("del"),
    hget: mockOperation("hget"),
    hset: mockOperation("hset"),
    hdel: mockOperation("hdel"),
    hgetall: mockOperation("hgetall"),
    hmget: mockOperation("hmget"),
    zrange: mockOperation("zrange"),
    zadd: mockOperation("zadd"),
    zrem: mockOperation("zrem"),
    zcard: mockOperation("zcard"),
    incr: mockOperation("incr"),
    expire: mockOperation("expire"),
    ttl: mockOperation("ttl"),
    exists: mockOperation("exists"),
    scan: mockOperation("scan"),
    mget: mockOperation("mget"),
    lrange: mockOperation("lrange"),
    pipeline: () => ({
      get: mockOperation("pipeline.get"),
      set: mockOperation("pipeline.set"),
      del: mockOperation("pipeline.del"),
      hget: mockOperation("pipeline.hget"),
      hset: mockOperation("pipeline.hset"),
      hdel: mockOperation("pipeline.hdel"),
      hmget: mockOperation("pipeline.hmget"),
      zadd: mockOperation("pipeline.zadd"),
      zrem: mockOperation("pipeline.zrem"),
