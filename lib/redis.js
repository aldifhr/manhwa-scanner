...
// Incremental scraping timestamp tracking - Optimized with Hash sharding
// Uses Redis Hash per first letter to reduce key count and improve memory efficiency
// Hash key: scrape:lastChecks:{firstLetter}, Field: titleKey, Value: timestamp

const LAST_CHECK_HASH_PREFIX = "scrape:lastChecks";
const LAST_CHECK_TTL_MS = 24 * 60 * 60 * 1000; // 24 hours in ms

/**
 * Get hash key for a title (sharded by first letter)
 * Example: "a dragonslayer..." → "scrape:lastChecks:a"
 */
function getLastCheckHashKey(titleKey) {
  const firstChar = titleKey?.charAt(0)?.toLowerCase() || "_";
  // Handle non-alphabetic characters with "_" bucket
  const shard = /^[a-z]$/.test(firstChar) ? firstChar : "_";
  return `${LAST_CHECK_HASH_PREFIX}:${shard}`;
}
 ...
 * Groups by hash key for efficient HMGET operations
 */
export async function batchGetLastScrapeChecks(titleKeys) {
  if (!titleKeys || titleKeys.length === 0) return [];
  ...
  for (const key of titleKeys) {
    ...
    byShard.get(hashKey).push(key);
  }

  // Fall back to individual operations if pipeline not available
  if (typeof redis.pipeline !== "function") {
    const results = await Promise.all(
      titleKeys.map(async (key) => {
        const hashKey = getLastCheckHashKey(key);
        return await redis.hget(hashKey, key);
      }),
    );
    return results;
  }

  // Use pipeline with HMGET for each shard
  const pipeline = redis.pipeline();
  const shardKeys = [];
  for (const [hashKey, fields] of byShard) {
    shardKeys.push({ hashKey, fields });
  ...
  results.forEach((shardResult, i) => {
    const { fields } = shardKeys[i];
    fields.forEach((field, j) => {
      resultMap.set(field, shardResult?.[j] ?? null);
    });
  });

  return titleKeys.map((key) => resultMap.get(key) ?? null);
}

/**
 * Batch set last scrape check timestamps using pipeline
 * Groups by hash key for efficient HMSET operations
...
export async function batchSetLastScrapeChecks(
  ...
  const ttlSeconds = Math.floor(LAST_CHECK_TTL_MS / 1000);
  const hasHpexpire = typeof redis.hpexpire === "function";
  const hasHexpire = typeof redis.hexpire === "function";
  ...
  for (const key of titleKeys) {
    const hashKey = getLastCheckHashKey(key);
    if (!byShard.has(hashKey)) {
      byShard.set(hashKey, { fields: {}, keys: [] });
    }
    byShard.get(hashKey).fields[key] = ts;
    byShard.get(hashKey).keys.push(key);
  }
  ...
  if (typeof redis.pipeline !== "function") {
    await Promise.all(
      titleKeys.map(async (key) => {
        const hashKey = getLastCheckHashKey(key);
        await redis.hset(hashKey, { [key]: ts });
        // Try per-field TTL
        try {
          if (hasHpexpire) {
            await redis.hpexpire(hashKey, key, LAST_CHECK_TTL_MS);
          } else if (hasHexpire) {
            await redis.hexpire(hashKey, ttlSeconds, "FIELDS", 1, key);
          }
        } catch {
          // Ignore TTL errors for older Redis
        }
      }),
    );
    return;
  ...
  // Use pipeline with HSET + optional HEXPIRE per field
  const pipeline = redis.pipeline();
  for (const [hashKey, { fields, keys }] of byShard) {
    pipeline.hset(hashKey, fields);
    // Add HEXPIRE per field if supported (Redis 7.4+)
    if (hasHpexpire) {
      for (const key of keys) {
        pipeline.hpexpire(hashKey, key, LAST_CHECK_TTL_MS);
      }
    } else if (hasHexpire) {
...
export async function cleanupStaleLastChecks(
  ...
  let totalDeleted = 0;
  const shards = "abcdefghijklmnopqrstuvwxyz_".split("");

  for (const shard of shards) {
    const hashKey = `${LAST_CHECK_HASH_PREFIX}:${shard}`;
    const toDelete = [];

    // Use HSCAN to iterate without loading entire hash
    let cursor = 0;
    do {
      const result = await redis.hscan(hashKey, cursor, { count: 100 });
 ...
 * Returns total entries and per-shard breakdown
 */
export async function getLastCheckStats() {
  const stats = { total: 0, shards: {} };
  const shards = "abcdefghijklmnopqrstuvwxyz_".split("");
...
export async function loadWhitelist() {
  try {
    ...
    if (index && index.length > 0) {
      ...
      console.log(
        `[loadWhitelist] Index=${index.length} HMGET_Type=${typeof data} Array=${Array.isArray(data)} Final=${list.length}`,
      );
      return list;
    }
    ...
    if (rawLegacy) {
      ...
      if (list.length > 0) {
        ...
        console.log(
          `[loadWhitelist] Migrated ${list.length} items to new structure.`,
        );
      }
      return list;
    }
...
export async function getNotificationChannelFromStore(client, guildId) {
  ...
  await client.hset(CHANNEL_HASH_KEY, { [field]: value }).catch((err) => {
    console.error(
      `[redis] Failed to migrate channel for guild ${field}:`,
      err.message,
    );
  });
  return value;
...
export async function setNotificationChannelInStore(
  ...
  await client.del(`${CHANNEL_KEY_PREFIX}${field}`).catch((err) => {
    console.error(
      `[redis] Failed to delete legacy channel for guild ${field}:`,
      err.message,
    );
  });
}
...
export async function getAllGuildChannelsFromStore(client) {
  ...
  await hydrateChannelHash(client, legacy).catch((err) => {
    console.error("[redis] Failed to hydrate channel hash:", err.message);
  });
  return legacy;
}
...
export async function writeObjectCache(client, key, payload, cacheTtl) {
  await client.set(key, payload, { ex: cacheTtl }).catch((err) => {
    console.error(`[redis] Failed to write cache ${key}:`, err.message);
  });
}
