...
export async function appendLiveEvent(redisClient, event = {}) {
  ...
  try {
    ...
    pipeline.expire(LIVE_EVENTS_KEY, 3600 * 24);
    await pipeline.exec();
    return true;
  } catch (err) {
...
export async function loadWhitelist(client = redis) {
  try {
    ...
    const raw = await client.hmget(WHITELIST_DATA_KEY, ...index);
...
export async function saveWhitelist(list, client = redis) {
  ...
  const sanitized = normalized.filter((item) => {
    ...
    if (!tk) {
      logger.warn({ item }, "[saveWhitelist] Skip item tanpa titleKey valid");
      return false;
    }
    return true;
  });
  ...
  if (!sanitized.length) {
    await Promise.all([
      client.del(WHITELIST_DATA_KEY),
      client.del(WHITELIST_INDEX_KEY),
    ]);
    return [];
  }

  const previousIndex = await client.zrange(WHITELIST_INDEX_KEY, 0, -1);
  const nextKeys = [];
  const dataMap = {};
  const zaddEntries = [];

  sanitized.forEach((item, i) => {
    const tk = normalizeTitleKey(item.title);
    nextKeys.push(tk);
    dataMap[tk] = JSON.stringify(item);
    zaddEntries.push({ score: i, member: tk });
  });

  const removedKeys = (previousIndex || []).filter((key) => !nextKeys.includes(key));

  const pipeline = client.pipeline();
  pipeline.hset(WHITELIST_DATA_KEY, dataMap);

  if (removedKeys.length) {
    pipeline.hdel(WHITELIST_DATA_KEY, ...removedKeys);
    pipeline.zrem(WHITELIST_INDEX_KEY, ...removedKeys);
  }

  if (zaddEntries.length) {
    for (const entry of zaddEntries) {
...
export async function getWhitelistCount(client = redis) {
  try {
    return await client.zcard(WHITELIST_INDEX_KEY);
  } catch (err) {
    logger.warn({ err: err.message }, "[getWhitelistCount] Failed");
    return 0;
  }
}
...
export async function loadSourceHealthSnapshot(client, sourceKeys = SOURCE_KEYS) {
  ...
  const entries = sourceKeys.map((source) => {
    ...
    try {
      return [source, typeof raw === "string" ? JSON.parse(raw) : raw];
    } catch {
      return [source, null];
    }
  });
...
export async function batchClaimPendingChapters(
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (claimable) claimIndices.push(i);
  }
  ...
  if (claimIndices.length > 0) {
    ...
    await claimPipeline.exec();
    claimIndices.forEach((idx) => {
      results[idx] = true;
    });
  }
...
export async function readRecentChapters(client = redis, limit = 50) {
  const raw = await client.lrange(RECENT_CHAPTERS_KEY, 0, limit - 1);
