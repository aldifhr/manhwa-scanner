...
export async function getLastScrapeCheck(titleKey) {
  return await redis.get(`scrape:lastCheck:${titleKey}`);
}

export async function setLastScrapeCheck(titleKey, timestamp = Date.now()) {
  await redis.set(`scrape:lastCheck:${titleKey}`, timestamp, { ex: 86400 });
}
...
export async function batchGetLastScrapeChecks(titleKeys) {
  ...
  if (typeof redis.pipeline !== "function") {
    return await Promise.all(
      titleKeys.map((key) => redis.get(`scrape:lastCheck:${key}`)),
    );
  }

  const pipeline = redis.pipeline();
  for (const key of titleKeys) {
    pipeline.get(`scrape:lastCheck:${key}`);
  }
  return await pipeline.exec();
}
...
export async function batchSetLastScrapeChecks(
  ...
  if (typeof redis.pipeline !== "function") {
    await Promise.all(
      titleKeys.map((key) =>
        redis.set(`scrape:lastCheck:${key}`, timestamp, { ex: 86400 }),
      ),
    );
    return;
  }
  ...
  const pipeline = redis.pipeline();
  for (const key of titleKeys) {
    pipeline.set(`scrape:lastCheck:${key}`, timestamp, { ex: 86400 });
  }
  await pipeline.exec();
}
