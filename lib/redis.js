...
export async function fetchDashboardSnapshot() {
  ...
  pipeline.hgetall("sources:health");         // 1: Source health map
  pipeline.get("health:recommendations");    // 2: Health recommendations
  pipeline.get("health:last-check");         // 3: Last health check time

  // Memory optimization: only fetch full hash if size is reasonable
  if (snapshotLen < 500) {
    pipeline.hgetall(RECENT_CHAPTERS_KEY);       // 4: Recent chapters (Hash)
  } else {
    pipeline.hkeys(RECENT_CHAPTERS_KEY);         // 4: Just keys (lighter) if bloated
  }
  ...
  // Process recent chapters from Hash format
  ...
  const liveEvents = (results[6] || []).map((v) =>
    typeof v === "string" ? JSON.parse(v) : v,
  );
  const queueLength = results[7] || 0;
  const rawQueueItems = results[8] || [];
  const queueItems = rawQueueItems.map(v => typeof v === "string" ? JSON.parse(v) : v);
