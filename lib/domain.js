...
export function normalizeSourceUrl(url = "") {
  ...
  const shigDomains =
    /^(https?:\/\/(?:[a-z0-9-]+\.)?(?:shngm\.id|shinigami\.asia|shinigami-id\.com|shinigami\.moe|shinigami\.ink))(?:[/:?#]|$)/i;
  if (shigDomains.test(normalized)) {
    return normalized.replace(/^https?:\/\/[^/]+/i, shigBase);
  }

  if (/^https?:\/\/(?:[a-z0-9-]+\.)?(ikiru\.wtf|komikcast\.com|komikcast\.site)(?:[/:?#]|$)/i.test(normalized)) {
...
export function isSameNormalizedTitle(left = "", right = "") {
  ...
  if (leftKey === rightKey || compactTitleKey(leftKey) === compactTitleKey(rightKey)) return true;
