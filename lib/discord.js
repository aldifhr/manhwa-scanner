...
export async function editInteractionResponseWithComponents(token, content, components) {
  try {
    await axios.patch(
      `https://discord.com/api/v10/webhooks/${APP_ID}/${token}/messages/@original`,
      { content, components },
...
export async function editWithComponents(payload, content, components) {
  try {
    await axios.patch(
