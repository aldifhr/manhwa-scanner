...
export async function sendDiscordEmbed(
  ...
  if (titleKey && chapterKey) {
    const { checkChapterSentPermanent } = await import("./supabase.js");
    const alreadySentL2 = await checkChapterSentPermanent(titleKey, chapterKey, channelId);
    if (alreadySentL2) {
      logger.info(
        { channelId, title, chapter },
        "Skipped duplicate Discord embed (Supabase L2 dedupe)",
      );
      // To save future Supabase calls, optionally rewrite back to Redis (Hydration)
      if (redis && typeof redis.set === "function") {
        const dedupeKey = `discord:dedupe:${channelId}:${titleKey}:${chapterKey}`;
        redis.set(dedupeKey, Date.now().toString(), { nx: true, ex: 120 }).catch(() => {});
      }
      return { success: true, status: 200, channelId, deduplicated: true }; // Return true to indicate it's "resolved"
    }
  }
