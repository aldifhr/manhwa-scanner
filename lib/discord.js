...
export async function sendDiscordEmbed(
  ...
  const truncatedDescription = description.length > DISCORD_EMBED_DESCRIPTION_LIMIT
    ? description.substring(0, DISCORD_EMBED_DESCRIPTION_LIMIT - 3) + "..."
    : description;
  ...
  const sendTasks = items.map((item, index) =>
    limit(async () => {
      ...
      if (deduplicate) {
        ...
        await sendDiscordEmbed(data, channelId, redisClient, mentions);
      }
