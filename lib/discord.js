...
export async function editInteractionResponse(token, content) {
  ...
  const userId =
    typeof token === "object" ? token.member?.user?.id || token.user?.id : null;
  const appId =
    typeof token === "object" && token.application_id
  ...
  try {
    ...
    if (status === 404 || status === 401) {
      // Interaction token expired or invalid — fail silently.
      // Do NOT fall back to a public channel message as that would be visible to everyone.
      logger.warn(
        { status, appId: appId ? "set" : "missing" },
        "[editInteractionResponse] Interaction token expired or invalid, skipping response",
      );
      return;
    }
...
export async function editInteractionResponseWithComponents(
  ...
  try {
    ...
    if (status === 404 || status === 401) {
      ...
      logger.warn(
        { status },
        "[editInteractionResponseWithComponents] Interaction token expired or invalid, skipping",
      );
      return;
    }
    logger.error(
      { err: err.message, status },
      "[editInteractionResponseWithComponents] Failed",
    );
    throw err;
  }
}
...
export async function sendDiscordEmbed(
  ...
  const truncatedTitle =
    title.length > DISCORD_EMBED_TITLE_LIMIT
  ...
  if (redis && typeof redis.set === "function") {
    if (titleKey && chapterKey) {
      ...
      try {
        ...
        logger.warn(
          { err: err.message },
          "Discord dedupe guard failed, continuing send",
        );
      }
    }
  }

  const unixTime = data.updatedTime
    ? Math.floor(new Date(data.updatedTime).getTime() / 1000)
    : null;
  const updatedString =
    unixTime && !Number.isNaN(unixTime) ? `<t:${unixTime}:R>` : "Unknown";
