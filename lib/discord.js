...
export async function sendDiscordEmbed(data, channelId, redis = null) {
  ...
  const synopsisFromData = shortSynopsis(data.description);
  const synopsisFetched = synopsisFromData
    ? null
    : shortSynopsis(await fetchDescription(safeMangaUrl, redis));
  const synopsis = synopsisFromData || synopsisFetched;
