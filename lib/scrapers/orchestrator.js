...
import { normalizeSource, normalizeTitleKey, parseIkiruDatetime } from "./shared.js";
  ...
  try {
    ...
    const secondaryStateEntries = await Promise.all(
      secondarySources.map(async (source) => {
        const preferredTitleKeys = preferredSecondaryTitleKeysBySource[source];
    ...
    for (const item of allResults) {
      const chapterKey = String(item.url || "").replace(/\/+$/, "").toLowerCase().trim();
