...
import {
  scrapeIkiruUpdatesWithMeta as scrapeIkiruFn,
  fetchIkiruMetadata,
} from "./ikiru.js";
import {
  scrapeSecondarySourceUpdates as scrapeSecondaryFn,
...
async function getHibernatingTitleKeys(redis, titleKeys, options = {}) {
  ...
  let timestamps = await redis.hmget(MANGA_LAST_UPDATES_KEY, ...titleKeys);
...
export async function orchestrateScrapeSources({
  ...
  fetchIkiruMetadataFn = fetchIkiruMetadata,
  fetchSecondaryMetadataFn = fetchSecondaryMetadata,
  logger = moduleLogger,
} = {}) {
  const { lifecycle, startTime = Date.now(), deadlineMs = 0 } = options;
  const deadline = deadlineMs > 0 ? startTime + deadlineMs : 0;
  // Margin for overall scrape process (leave room for dispatch)
  const SCRAPE_SAFETY_MARGIN_MS = 10000;
  ...
  try {
    ...
    // --- STEP 1 & 2: PARALLEL INITIALIZATION (Hibernation & Incremental) ---
    ...
    // Run Hibernation and Incremental checks in parallel
    const [skipTitleKeys, ikiruIncrementalFiltered, secondaryIncrementalFiltered] = await Promise.all([
      ...
      useIncremental && preferredIkiruTitleKeys.size > 0
        ? applyIncrementalFilter(preferredIkiruTitleKeys, redis, logger)
        : null,
      Promise.all(
        secondarySources.map(async (source) => {
          const matcher = preferredSecondaryMatchersBySource[source];
          const results = { source, titleKeys: null, urlKeys: null };
          if (useIncremental) {
            [results.titleKeys, results.urlKeys] = await Promise.all([
              matcher.titleKeys.size > 0 ? applyIncrementalFilter(matcher.titleKeys, redis, logger) : null,
              matcher.urlKeys.size > 0 ? applyIncrementalFilter(matcher.urlKeys, redis, logger) : null,
            ]);
          }
          return results;
        }),
    ...
    if (ikiruIncrementalFiltered) {
      ...
      // Re-apply hibernation delete if any woke up title was actually filtered out by incremental
      if (skipTitleKeys.size > 0) {
        for (const tk of skipTitleKeys) preferredIkiruTitleKeys.delete(tk);
      }
    }

    for (const filterRes of secondaryIncrementalFiltered) {
      const matcher = preferredSecondaryMatchersBySource[filterRes.source];
      if (filterRes.titleKeys) matcher.titleKeys = filterRes.titleKeys;
      if (filterRes.urlKeys) matcher.urlKeys = filterRes.urlKeys;
      // Re-apply hibernation delete
      if (skipTitleKeys.size > 0) {
        for (const tk of skipTitleKeys) matcher.titleKeys.delete(tk);
      }
    }
    // --- END INITIALIZATION ---
    ...
    const [ikiruResult, ...secondaryResults] = await Promise.all([
      ...
      (async () => {
        ...
        try {
          ...
          if (lifecycle) lifecycle.currentStep = "scraping_ikiru";
          ...
          if (useDedup) {
            ...
            out = await withTimeout(
              dedupedRequest(
                ...
                30000, // Cache lock TTL
              ),
              Math.max(2000, (deadline > 0 ? deadline - Date.now() : 20000)),
              "Ikiru Scraper hard timeout",
            );
          } else {
            out = await withTimeout(
              scrapeIkiruUpdatesWithMeta(
          ...
          if (redis && out.results?.length) {
            await appendLiveEvent(redis, {
              message: `Scraped ${out.results.length} items from Ikiru`,
              type: "info",
            }).catch(() => {});
          }
      ...
      ...secondarySources.map(async (source) => {
        ...
        // Deadline check for secondary sources
        if (deadline && Date.now() > deadline - SCRAPE_SAFETY_MARGIN_MS) {
          moduleLogger.warn({ source }, "skipping secondary source due to nearing deadline");
          return {
            source,
            results: [],
            state: {
              status: "skipped",
              count: 0,
              error: "Nearing execution deadline",
        ...
        if (lifecycle) lifecycle.currentStep = `scraping_${source}`;
        ...
        try {
            ...
            out = await withTimeout(
              dedupedRequest(
                ...
                () => scrapeSecondarySourceUpdates(
                  source,
                  { preferredMatcher: matcherSnapshot, redis, options, deadline },
                  logger,
                ),
                30000,
              ),
              Math.max(2000, (deadline > 0 ? deadline - Date.now() : 20000)),
              `Secondary Scraper (${source}) hard timeout`,
            );
          } else {
            out = await withTimeout(
              scrapeSecondarySourceUpdates(
          ...
          if (redis && out.results?.length) {
            await appendLiveEvent(redis, {
              message: `Scraped ${out.results.length} items from ${source}`,
              type: "info",
            }).catch(() => {});
          }
    ...
    // --- STEP 2.5: METADATA ENRICHMENT ---
    if (scrapedChapters.length > 0) {
      if (lifecycle) lifecycle.currentStep = "enriching_metadata";

      // Ensure every chapter has a titleKey for metadata mapping
      ...
      if (redis) {
        const cachedResults = await Promise.all(
          uniqueTitleKeys.map((tk) => getMangaMetadata(redis, tk)),
        );
      ...
      const missingTitleKeys = uniqueTitleKeys.filter(
        (tk) => !metadataMap.has(tk),
      );
      if (missingTitleKeys.length > 0) {
        logger.info(
          { missing: missingTitleKeys.length },
          "fetching missing manga metadata",
        );

        // Map titleKey to one chapter for metadata fetching
        const missingChapters = missingTitleKeys.map((tk) =>
          scrapedChapters.find((ch) => ch.titleKey === tk),
        );

        await Promise.all(
          missingChapters.map(async (ch) => {
            let meta = null;
            const source = normalizeSource(ch.source);

            try {
              if (source === "ikiru") {
                meta = await fetchIkiruMetadataFn(ch.mangaUrl, redis);
              } else if (
                source === "shinigami_project" ||
                source === "shinigami_mirror"
              ) {
                // Use mangaId from chapter object (set by secondary.js fix).
                // Fallback: safely parse the last path segment from mangaUrl
                // using URL API to handle trailing slashes and query strings.
                let mangaId = ch.mangaId ?? null;
                if (!mangaId && ch.mangaUrl) {
                  try {
                    const pathname = new URL(ch.mangaUrl).pathname;
                    mangaId = pathname.split("/").filter(Boolean).pop() ?? null;
                  } catch {
                    mangaId = null;
                  }
                }
                meta = await fetchSecondaryMetadataFn(source, mangaId, redis);
              }

              if (meta) {
                metadataMap.set(ch.titleKey, meta);
                if (redis) {
                  await setMangaMetadata(redis, ch.titleKey, meta);
                }
              }
            } catch (enrichErr) {
              logger.warn(
                { err: enrichErr.message, title: ch.title },
                "failed to enrich metadata",
              );
            }
          }),
        );
    ...
    // Optimized: Pre-calculate timestamps and numbers to avoid calculating inside O(n log n) sort
    const enrichedChapters = scrapedChapters.map(ch => ({
      ...ch,
      _timeCache: safeParseDate(ch.updatedTime)?.getTime(),
      _titleCache: (ch.title || "").toLowerCase(),
      _chapterNum: getChapterNumber(ch.chapter) || 0,
    }));
    ...
    // Replace original array contents with sorted and undecorated items
    scrapedChapters.length = 0;
    scrapedChapters.push(
      ...enrichedChapters.map(({ _timeCache, _titleCache, _chapterNum, ...item }) => item),
    );
