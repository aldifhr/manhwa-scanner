...
export async function orchestrateScrapeSources({
  ...
  try {
    ...
    // Declare these BEFORE try block so they're accessible in catch
    const sourceStates = {
    ...
    const [ikiruResult, ...secondaryResults] = await Promise.all([
      ...
      (async () => {
        try {
          ...
          if (out.state?.status !== "error" && preferredIkiruTitleKeys.size > 0) {
      ...
      ...secondarySources.map(async (source) => {
        ...
        try {
          ...
          if (preferredMatcher.titleKeys.size > 0) {
