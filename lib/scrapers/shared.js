...
export const LATEST_URL =
  process.env.IKIRU_LATEST_URL || `${SITE_URL}latest-update/`;
...
export const IKIRU_CHAPTER_LIST_MAX_PAGES = Number(
  process.env.IKIRU_CHAPTER_LIST_MAX_PAGES || 3,
);
...
async function releaseCookieLock(redis, token) {
  const current = await redis.get(COOKIE_LOCK_KEY);
  if (current === token) {
    await redis.del(COOKIE_LOCK_KEY);
  }
}
...
export async function getCookie(redis = null) {
  ...
  try {
    ...
    console.warn("[getCookie] Redis operation failed:", err.message);
    return process.env.IKIRU_COOKIE || "";
  }
}
...
export function parseRelativeTimeText(raw) {
  ...
  const unit = m[2];
  const hours =
    unit.startsWith("day") || unit === "hari" ? amount * 24 : amount;
