...
export const SITE_URL = process.env.IKIRU_BASE_URL || "https://02.ikiru.wtf/";
...
export const SECONDARY_PUBLIC_BASE =
  (() => {
    const base = (process.env.SHINIGAMI_BASE_URL || process.env.SECONDARY_PUBLIC_BASE || "https://a.shinigami.asia").replace(/\/+$/, "");
    return base
      .replace(/^https?:\/\/(?:[a-z0-9-]+\.)?shngm\.id\b/i, base)
      .replace(/^https?:\/\/(?:[a-z0-9-]+\.)?shinigami\.asia\b/i, base);
  })();
