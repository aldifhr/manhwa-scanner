...
import { isSameNormalizedTitle, normalizeTitleKey } from "../domain.js";
import { normalizeSource, normalizeSourceUrl, getShinigamiPublicBase } from "../domain.js";
...
export const IKIRU_LATEST_MAX_PAGES = Number(
  process.env.IKIRU_LATEST_MAX_PAGES || 3,
);
...
export const SECONDARY_DETAIL_MAX_MANGA = Number(
  process.env.SECONDARY_DETAIL_MAX_MANGA || 3,
);
...
export const SECONDARY_CHAPTER_LIST_MAX_PAGES = Number(
  process.env.SECONDARY_CHAPTER_LIST_MAX_PAGES || 1,
);
...
export async function scrapeWithHeaders(url, redis = null, options = {}) {
  ...
  return withRetry(
    ...
    options.retries !== undefined ? options.retries : 1,
  );
}
...
/**
 * Abstract-like Base class for all scrapers to ensure a consistent interface.
 */
export class BaseScraper {
  ...
  /**
   * Main entry point for scraping updates. 
   * Must be implemented by subclasses.
   */
  async scrapeUpdates(whitelist = []) {
    throw new Error(`scrapeUpdates() not implemented in ${this.name}`);
  }

  /**
   * Scrapes full details for a single manga.
   * Must be implemented by subclasses.
   */
  async scrapeDetail(url) {
    throw new Error(`scrapeDetail() not implemented in ${this.name}`);
  }

  /**
   * Identifies if this scraper should handle a given URL.
   */
  canHandle(url) {
     return false;
  }
