...
const VALID_STATUSES = new Set(["Ongoing", "Completed", "Hiatus", "Updating"]);
const STATUS_MAP = { Updating: "Ongoing" };

const findStatusElement = ($vertical) =>
  $vertical
    .find("p.font-normal.text-xs, .text-sm.text-text.line-clamp-1, .line-clamp-1")
    .filter(
      (_, el) => {
        const text = $vertical.find(el).text().trim();
        return text && VALID_STATUSES.has(text);
      },
    )
    .first();

const extractStatusFromElement = ($element) => {
  const rawStatus = $element?.text().trim() ?? "Unknown";
...
const extractStatusFromPage = ($, baseItem) => {
  if (baseItem?.status) return baseItem.status;
  ...
  return STATUS_MAP[rawStatus] || rawStatus || "Unknown";
};
