...
const extractMaxPageFromElement = ($el, maxPage) => {
  const text = $el.text().trim();
  const num = Number.parseInt(text, 10);
  if (Number.isFinite(num)) return Math.max(maxPage, num);

  const href = $el.attr("href");
  if (!href) return maxPage;

  const patterns = [/the_page=(\d+)/, /page=(\d+)/, /\/page\/(\d+)/];
  const match = patterns.map((p) => href.match(p)).find(Boolean);

  if (match) {
    const p = Number.parseInt(match[1], 10);
    if (Number.isFinite(p)) return Math.max(maxPage, p);
  }
...
const findMaxPage = ($) => {
  let maxPage = 1;
  $(".pagination a, .page-numbers, a.page-numbers, .pagination-next").each(
    (_, el) => {
      maxPage = extractMaxPageFromElement($(el), maxPage);
    },
  );
  return Math.max(maxPage, 50);
};

const extractMangaCardData = ($card) => {
  const $vertical = $card.children("div").first();
  if (!$vertical.length) return null;
  ...
  if (isMangaType(typeText)) return null;
...
const parseMangaResults = ($) => {
  const results = [];
  $("#search-results")
    .children()
    .each((_, card) => {
      const data = extractMangaCardData($(card));
      if (data) results.push(data);
    });
  return results;
};

const fetchRandomPage = async (redis, pageNum) => {
  const url = `${SITE_URL}/${RANDOM_MANGA_PATH}&the_page=${pageNum}`;
  const headers = await baseHeaders(redis);
  const res = await retryAsync(
    () =>
      fetchWithTimeout(
        url,
        { headers, timeout: FASTCRON_SAFE_CONFIG.REQUEST_TIMEOUT },
        `Random manga page fetch timeout for page ${pageNum}`,
      ),
    retryConfig,
  );
...
export async function fetchRandomIkiruManga(redis = null) {
  try {
    const initialUrl = `${SITE_URL}/${RANDOM_MANGA_PATH}&the_page=1`;
    const headers = await baseHeaders(redis);
    ...
    const maxPage = findMaxPage($initial);
    ...
    if (randomPage === 1) {
      ...
      $ = await fetchRandomPage(redis, randomPage);
    }

    const results = parseMangaResults($);
