...
import { IKIRU_CONFIG, SCRAPER_LOOKBACK_HOURS } from "../config.js";
...
// Dynamically calculate buffer: we need to ensure the NEXT request has enough time to complete
// before TOTAL_TIMEOUT is reached. A request could take up to REQUEST_TIMEOUT.
const isApproachingTimeout = (startTime, buffer = FASTCRON_SAFE_CONFIG.REQUEST_TIMEOUT + 500) =>
  Date.now() - startTime > FASTCRON_SAFE_CONFIG.TOTAL_TIMEOUT - buffer;

const isValidMangaInfo = (info) => info?.title && info?.$vertical?.length;

const isChapterFresh = (parsedUpdated, lookbackHours = SCRAPER_LOOKBACK_HOURS) => {
  if (!parsedUpdated) return false;
  const diffHours = (Date.now() - parsedUpdated.getTime()) / 3600000;
  return diffHours <= lookbackHours;
};
...
function parseChapterRow($link, mangaInfo, seen, options = {}) {
  const chapterText = getChapterText($link);
  const updatedText = getUpdatedTime($link);
  const parsedUpdated = parseDateWithFallback(updatedText) || parseLooseRelativeTime(updatedText);

  if (!isChapterFresh(parsedUpdated, options.lookbackHours)) {
  ...
  const rawResult = buildChapterResult(mangaInfo, chapterText, url, parsedUpdated);
  const result = validateChapter(rawResult, logger);
...
function parsePage($, seen, options = {}) {
  ...
  const results = $cards
    .map((_, card) => {
      ...
      return mangaInfo.$vertical
        .find("a[href*='/chapter-']")
        .map((_, el) => {
          const parsed = parseChapterRow($(el), mangaInfo, seen, options);
...
export function collectIkiruRecentChaptersFromMangaPage(
  ...
  const lookbackHours = options.lookbackHours ?? SCRAPER_LOOKBACK_HOURS;
  ...
  $(CHAPTER_ROW_SELECTORS).each((_, el) => {
    ...
    if (!isChapterFresh(parsedUpdated, lookbackHours)) return;

    const key = createChapterKey(title, chapterText);
    if (!checkAndMarkSeen(key, seenKeys)) return;

    const rawResult = buildChapterResult(
      {
        title,
        cover,
    ...
    const validResult = validateChapter(rawResult, logger);
    if (validResult) results.push(validResult);
  });
...
export function collectIkiruRecentChaptersFromAjaxHtml(
  ...
  $(AJAX_CHAPTER_SELECTORS).each((_, el) => {
    ...
    if (!isChapterFresh(parsedUpdated, lookbackHours)) {
...
const fetchWithTimeout = async (url, config, timeoutMsg, deadline = 0) => {
  const timeoutMs = config.timeout || FASTCRON_SAFE_CONFIG.REQUEST_TIMEOUT;

  // Calculate remaining time based on deadline
  let finalTimeout = timeoutMs;
  if (deadline > 0) {
    const remaining = deadline - Date.now();
    if (remaining <= 0) throw new Error("deadlineExceeded");
    finalTimeout = Math.min(timeoutMs, remaining);
  }

  return withTimeout(
    axios.get(url, { ...config, timeout: finalTimeout }),
    finalTimeout + 1000, // Buffer for Promise.race wrap
    timeoutMsg,
  );
};
...
export async function fetchIkiruRecentChaptersFromAjax(
  ...
  // Use pLimit for controlled parallel page fetching
  const limit = pLimit(3);
  const pageIndices = Array.from({ length: maxPages }, (_, i) => i + 1);

  // We process pages in parallel, but we need to keep track if we hit an old chapter
  // to avoid fetching unnecessary further pages (though in parallel some might already be inflight)
  let stopPaging = false;

  await Promise.all(
    pageIndices.map((page) =>
      limit(async () => {
        if (stopPaging || isApproachingTimeout(startTime, 3000)) return;
        ...
        try {
          ...
          if (results.length) {
            // Use locked push if needed, but array.push in Promise.all is generally safe for simple values
            collected.push(...results);
          }

          if (!results.length || foundOlderThan24h) {
            stopPaging = true;
          }
        } catch (err) {
          logger.warn(
            `[fetchIkiruRecentChaptersFromAjax] Failed for ${mangaUrl} page ${page}:`,
            err.message,
          );
          stopPaging = true;
        }
...
export async function fetchIkiruRecentChaptersFromMangaPage(
  ...
  try {
    ...
    return ajaxResults.length
      ? ajaxResults
      : collectIkiruRecentChaptersFromMangaPage($, mangaUrl, baseItem, seen, options);
  } catch (err) {
    logger.warn(
  ...
  await Promise.all(
    batch.map((item) =>
      limit(async () => {
        ...
        const isNew = await checkUpdateClaim(redis, item.title, item.chapter);
        if (!isNew) {
          logger?.info({ title: item.title, chapter: item.chapter }, "ikiru expansion skipped: already claimed by another source");
          return;
        }
...
export async function expandIkiruUpdatesFromDetailPages(
  ...
  for (const batch of chunked(candidates, 20)) {
    await processCandidateBatch(batch, redis, replacementMap, limit, startTime, options);
...
/**
 * Fetch detailed metadata for an Ikiru manga
 */
export async function fetchIkiruMetadata(mangaUrl, redis = null) {
  if (!mangaUrl) return null;
  ...
  try {
    ...
    // Metadata selectors refined from live HTML
    const synopsis =
      normalizeText($('div[itemprop="description"]').not(".hidden").text()) ||
      normalizeText($('div[itemprop="description"]').first().text()) ||
      normalizeText($('meta[name="description"]').attr("content") || "");

    const rating = normalizeText($('div[itemprop="ratingValue"]').attr("content") ||
                                $(".bg-clip-text").first().text() || "");
    ...
    const cover = $('meta[property="og:image"]').attr("content") ||
                  cleanImageUrl(toAbsoluteUrl($("img.wp-post-image").first().attr("src"), mangaUrl));
    ...
    logger.warn({ err: err.message, mangaUrl }, "Failed to fetch Ikiru metadata");
    return null;
  }
}
    ...
    const res = await retryAsync(
        ...
        ),
      {
        ...
        shouldRetry: (err) => {
          ...
          return !isApproachingTimeout(startTime, FASTCRON_SAFE_CONFIG.REQUEST_TIMEOUT + 500);
        },
      },
    );
...
export async function scrapeIkiruUpdatesWithMeta(
  ...
  const { skipExpansion = false } = options;
  ...
  for (const pageData of pageUrls) {
    ...
    const pageResults = parsePage($, seenChapterKeys, { ...options, lookbackHours });
