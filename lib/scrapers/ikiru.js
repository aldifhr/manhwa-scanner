...
export async function fetchRandomIkiruManga(redis = null) {
  try {
    ...
    $(containerSelector).children().each((_, card) => {
      ...
      const typeText = $vertical.find(".type, .manga-type, [class*='type']").text().trim().toLowerCase();
    ...
    // Secondary attempt if no results on random page, try page 1
    if ($ !== $initial) {
      const firstResults = [];
      $initial(containerSelector).children().each((_, card) => {
        const $card = $(card);
        const $vertical = $card.children("div").first();
        if ($vertical.length) {
          const title = $vertical.find("h1").first().text().trim();
