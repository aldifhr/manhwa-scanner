...
const FASTCRON_SAFE_CONFIG = {
  REQUEST_TIMEOUT: Math.max(3000, Number(process.env.IKIRU_REQUEST_TIMEOUT_MS) || 5000), // 5s default, min 3s
  MAX_RETRIES: Math.min(3, Math.max(1, Number(process.env.IKIRU_MAX_RETRIES) || 2)), // 2 retries default
  RETRY_DELAY: 500,
  BATCH_SIZE: 10,
  TOTAL_TIMEOUT: Math.max(10000, Number(process.env.IKIRU_TOTAL_TIMEOUT_MS) || 15000), // 15s default
  MAX_CHAPTERS_PER_RUN: Math.min(10, Math.max(3, Number(process.env.IKIRU_MAX_CHAPTERS) || 5)),
};
