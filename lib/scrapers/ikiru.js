...
const FASTCRON_SAFE_CONFIG = {
  REQUEST_TIMEOUT: Math.max(3000, Number(process.env.IKIRU_REQUEST_TIMEOUT_MS) || 4000), // 4s default
...
const processSinglePage = async (
  ...
  // Cache key: 60-second TTL for same page
  const cacheKey = `ikiru:latest:page:${page}:${Math.floor(Date.now() / 60000)}`;
  ...
  try {
    ...
    // Cache the response (75s TTL to cover 60s window + buffer)
    if (redis) {
      redis.set(cacheKey, res.data, { ex: 75 }).catch((err) => {
...
export async function scrapeIkiruUpdatesWithMeta(
  ...
  for (const resp of pageResponses) {
    ...
    // Early exit: if page 1 has no results, skip remaining pages
    if (resp.page === 1 && pageResults.results.length === 0) {
      logger?.info("First page empty, skipping remaining pages");
      break;
    }
  }
...
// Fetch recent chapters (<24h) from manga detail page
...
export async function fetchRecentChaptersFromMangaPage(mangaUrl, redis = null) {
  ...
  try {
    ...
    const baseItem = { title: extractTitleFromPage($, {}) };
    const chapters = collectIkiruRecentChaptersFromMangaPage($, mangaUrl, baseItem, new Set());

    return chapters;
  } catch (err) {
    console.error("[fetchRecentChaptersFromMangaPage] Error:", err.message);
    return [];
  }
}
