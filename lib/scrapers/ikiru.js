...
const FASTCRON_SAFE_CONFIG = {
  REQUEST_TIMEOUT: Math.max(5000, Number(process.env.IKIRU_REQUEST_TIMEOUT_MS) || 8000), // 8s default
...
export async function searchIkiru(query, _options = {}, redis = null) {
  ...
  try {
    ...
    const pushResult = (input = {}) => {
      ...
      const title = normalizeText(input.title) || inferTitleFromMangaUrl(mangaUrl);
      if (!title) return;
