...
async function fetchIkiruRecentChaptersFromAjax(
  ...
  for (
    let page = 1;
    page <= Math.max(1, IKIRU_CHAPTER_LIST_MAX_PAGES);
    page++
  ) {
    ...
    if (!candidates.length) {
      ...
      if (latestChapterLink) {
        try {
          ...
          const chapterRes = await scrapeWithHeaders(chapterUrl, redis, {
            timeout: 5000,
          });
...
export async function fetchRandomIkiruManga(redis = null) {
  try {
    ...
    const initialRes = await scrapeWithHeaders(initialUrl, redis, {
      timeout: 6000,
    });
    ...
    if (randPage > 1) {
      ...
      try {
        const res = await scrapeWithHeaders(url, redis, { timeout: 6000 });
...
export async function fetchDescription(mangaUrl, redis = null) {
  ...
  try {
    const res = await scrapeWithHeaders(mangaUrl, redis, { timeout: 5000 });
    const $ = cheerio.load(res.data);
    let desc =
