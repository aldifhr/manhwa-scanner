...
// Balanced mode: 10-12s target - fast but complete
const FASTCRON_SAFE_CONFIG = {
  REQUEST_TIMEOUT: 3000, // 3s timeout - balanced fail-fast
  MAX_RETRIES: 1, // 1 retry for reliability
  RETRY_DELAY: 500, // Short delay between retries
  BATCH_SIZE: 10, // High parallelism
  TOTAL_TIMEOUT: 12000, // 12s limit - seimbang cepat & lengkap
  MAX_CHAPTERS_PER_RUN: 5, // Recent 5 chapters
};
...
const findStatusElement = ($vertical) =>
  $vertical
    ...
    .filter((_, el) => VALID_STATUSES.has($vertical(el).text().trim()))
    .first();
