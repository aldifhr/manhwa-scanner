...
export async function getHibernatingTitleKeys(redis, titleKeys, options = {}) {
  ...
  const redisKeys = titleKeys.map(tk => `manga:last_update:${tk}`);
  const timestamps = await redis.mget(...redisKeys);
...
export async function touchMangaUpdate(redis, titleKey) {
  ...
  await redis.set(`manga:last_update:${titleKey}`, new Date().toISOString());
}
