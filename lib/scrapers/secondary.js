...
const FASTCRON_SAFE_CONFIG = {
  ...
  DIRECT_FALLBACK_MAX_URLS: Math.max(
    10,
    Number(process.env.SECONDARY_DIRECT_FALLBACK_MAX_URLS) || 50,
  ),
};
...
async function fetchSecondaryMangaDetail(apiBase, mangaId) {
  const endpoint = `${apiBase}/v1/manga/detail/${mangaId}`;
  const res = await fetchWithRetry(
    endpoint,
    JSON_HEADERS,
...
async function processRow(
  ...
  if (row?.__directFallback) {
    ...
    try {
      ...
      directRows = await fetchSecondaryRecentChapters(API_BASE, row.manga_id, redis);
    } catch (err) {
      logger?.warn?.(
        {
          source: normalized,
