...
async function fetchDetailChapters(
  ...
  try {
    ...
    // Note: detailState.count was already incremented in processRow before calling this function
...
async function processRow(
  ...
  if (shouldUseDetail) {
    // Increment count atomically before fetching (race condition fix)
    // This ensures detailState.count reflects actual in-flight requests
    detailState.count += 1;
    ...
    try {
      ...
      // fetchDetailChapters handles its own errors, but reset count on unexpected error
      chapterRows = null;
    }
  }
