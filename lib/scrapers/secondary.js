...
const FASTCRON_SAFE_CONFIG = {
  ...
  DIRECT_FALLBACK_MAX_URLS: Math.max(
    5,
    Number(process.env.SECONDARY_DIRECT_FALLBACK_MAX_URLS) || 12,
  ),
};
...
function mapChapterCandidate(chapter, fallbackRow = null, now = Date.now()) {
  const chapterId =
    chapter?.chapter_id ??
    chapter?.id ??
...
function extractDetailChaptersFromMangaDetail(payload, now = Date.now()) {
  ...
  if (Array.isArray(chapterArrayCandidates) && chapterArrayCandidates.length) {
    return chapterArrayCandidates
      .map((chapter) => mapChapterCandidate(chapter, detail, now))
      .filter(Boolean);
  }

  const single = mapChapterCandidate(detail, detail, now);
...
async function fetchSecondaryRecentChaptersFromMangaDetail(apiBase, mangaId) {
  ...
  return extractDetailChaptersFromMangaDetail(res?.data, Date.now());
}
...
async function fetchDetailChapters(
  ...
  try {
    ...
    if (!chapters?.length) {
      chapters = await fetchSecondaryRecentChapters(apiBase, mangaId, redis);
    }
...
function buildDirectUrlFallbackRows(
  ...
  return rows.slice(0, FASTCRON_SAFE_CONFIG.DIRECT_FALLBACK_MAX_URLS);
}
...
async function processRow(
  ...
  if (row?.__directFallback) {
    const htmlFallbackRows = await fetchFallbackChaptersFromSeriesHtml(
      mangaUrl,
      redis,
      now,
      logger,
    ...
    if (!Array.isArray(htmlFallbackRows) || !htmlFallbackRows.length) {
      return [];
    }
