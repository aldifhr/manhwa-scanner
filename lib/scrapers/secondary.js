...
import { chunk, filter, orderBy, uniqBy } from "../utils.js";
...
// Thorough scraping config: longer timeouts, more retries, more chapters
const FASTCRON_SAFE_CONFIG = {
  REQUEST_TIMEOUT: 10000, // Increased from 8000ms (wait longer for slow responses)
  MAX_RETRIES: 3, // Increased from 2 (more retries for reliability)
  RETRY_DELAY: 1500, // Increased from 1000ms (longer delay between retries)
  BATCH_SIZE: 6, // Moderate parallel processing
  TOTAL_TIMEOUT: 35000, // Increased to allow thorough scraping
  MAX_CHAPTERS_PER_RUN: 25, // Increased to get more chapters per run
};
...
async function fetchWithRetry(endpoint, headers, timeout, context = "") {
  const { retryAsync, withTimeout } = await import("../utils.js");
...
function transformChapterRows(rows, now = Date.now()) {
  return rows
    .map((row) => {
      ...
      if (!isWithinHours(parsedTime, 24, now)) return null;
...
function getFallbackChapters(row, shouldUseDetail) {
  if (!shouldUseDetail) {
    return null;
  }
...
function transformChapterResults(row, chapterRows, seen, now = Date.now()) {
  ...
  return chapterRows
    .map((chapterRow) => {
      ...
      if (!parsedTime || !isWithinHours(parsedTime, 24, now)) return null;
...
async function processRow(
  ...
  let chapterRows = shouldUseDetail
    ? await fetchDetailChapters(
      apiBase,
      row.manga_id,
      redis,
      metrics,
  ...
  if (!chapterRows) {
    chapterRows = getFallbackChapters(row, !shouldUseDetail);
  }
  ...
  return transformChapterResults(row, chapterRows, seen, Date.now());
}
