      ...
      async () =>
        withTimeout(
          httpGet(
            ...
            {
              retries: 0, // We handle retry with retryAsync
              baseDelayMs: 0,
            },
          ),
...
export async function scrapeSecondarySourceUpdates(
  ...
  { throwOnError = false, preferredMatcher = null } = {},
  logger = null,
) {
  ...
  try {
    ...
    const itemTasks = rows.map((row) =>
      limit(async () => {
        ...
        const latestParsed =
          parseIkiruDatetime(
            row?.latest_chapter_time || row?.updated_at || null,
          ) ||
          parseLooseRelativeTime(
            row?.latest_chapter_time || row?.updated_at || null,
          );
        ...
        for (const chapterRow of chapterRows) {
          ...
          const parsedTime =
            parseIkiruDatetime(
              chapterRow?.created_at || row?.updated_at || null,
            ) ||
            parseLooseRelativeTime(
              chapterRow?.created_at || row?.updated_at || null,
            );
          if (!parsedTime || (Date.now() - parsedTime.getTime()) / 3600000 > 24)
            continue;
    ...
    const taskResults = await Promise.all(itemTasks);
  ...
  for (let page = 1; page <= 4; page++) {
    try {
      ...
      const res = await retryAsync(
        async () =>
          withTimeout(
            ...
            FASTCRON_SAFE_CONFIG.REQUEST_TIMEOUT + 1000,
            `Search SHNGM timeout for ${keyword} page ${page}`,
          ),
        {
          maxAttempts: FASTCRON_SAFE_CONFIG.MAX_RETRIES,
          delay: FASTCRON_SAFE_CONFIG.RETRY_DELAY,
      ...
      for (const row of rows) {
        ...
        if (!(normTitle.includes(keyword) || keyword.includes(normTitle)))
          continue;
        ...
        results.push({
          ...
          updatedTime:
            (
              ...
              parseLooseRelativeTime(
                row?.latest_chapter_time || row?.updated_at,
              )
            )?.toISOString() || null,
          description: pickSecondaryDescription(row),
          source: normalized,
...
export async function fetchRandomShinigamiManga(source = "shinigami_project") {
  ...
  try {
    // 1. Fetch page 1 to get total pages and metadata (Filtering for Manhua=2, Manhwa=3)
    ...
    let payload = initialRes.data || {};

    // Only make a second request if the random page is > 1
    if (randPage > 1) {
      const endpoint = `${apiBase}/v1/manga/list?type=${type}&page=${randPage}&page_size=40&sort=latest&sort_order=desc${filterQuery}`;
      // Use withTimeout and retryAsync for FastCron safety
      const res = await retryAsync(
        async () =>
          withTimeout(
            ...
            FASTCRON_SAFE_CONFIG.REQUEST_TIMEOUT + 1000,
            `Random SHNGM page fetch timeout for page ${randPage}`,
          ),
        {
          maxAttempts: FASTCRON_SAFE_CONFIG.MAX_RETRIES,
          delay: FASTCRON_SAFE_CONFIG.RETRY_DELAY,
    ...
    // Safety filter: ensure no Manga (ID 1) slips through if the API filter isn't perfect
    rows = rows.filter((r) => {
    ...
    console.warn("[fetchRandomShinigamiManga] Error:", err.message);
    return null;
  }
}
