...
const logger = getLogger({ scope: "secondary" });
 ...
 */
async function fetchWithRetry(endpoint, headers, timeout, context = "", deadline = 0) {
  return retryAsync(
    async () =>
      withTimeout(
        httpGet(endpoint, { headers, timeout, deadline }, { retries: 0, baseDelayMs: 0 }),
        timeout + 1000,
        context,
      ),
    {
      ...
      backoff: 2,
      shouldRetry: (err) => {
        if (err?.message === "deadlineExceeded") return false;
        return true;
      },
    },
  );
...
async function fetchChapterPage(apiBase, mangaId, page, pageSize, deadline = 0) {
...
function mapChapterCandidate(
  chapter, fallbackRow = null, now = Date.now(), lookbackHours = FRESH_CHAPTER_HOURS_DEFAULT,
) {
  const chapterId =
    chapter?.chapter_id ??
    chapter?.id ??
...
export function extractDetailChaptersFromMangaDetail(
  payload, now = Date.now(), lookbackHours = FRESH_CHAPTER_HOURS_DEFAULT,
) {
...
async function fetchSecondaryRecentChapters(
  apiBase, mangaId, redis = null, lookbackHours = FRESH_CHAPTER_HOURS_DEFAULT, deadline = 0,
) {
...
export async function fetchSecondaryFullMangaInfo(apiBase, mangaId, deadline = 0) {
  ...
  const raw = res?.data ?? {};
  // The API structure can be nested { data: { data: ... } } or just { data: ... }
  const payload = raw?.data ?? raw;
  return {
    raw: payload,
    chapters: extractDetailChaptersFromMangaDetail(raw, Date.now(), SCRAPER_LOOKBACK_HOURS),
    meta: payload?.data ?? payload,
  };
}

/**
 * Fetch and extract normalized metadata for a Shinigami manga
 */
export async function fetchSecondaryMetadata(source, mangaId, redis = null) {
  if (!mangaId) return null;

  try {
    const apiBase = API_BASE;
    const { raw: payload } = await fetchSecondaryFullMangaInfo(apiBase, mangaId, 0);
    const data = payload?.data || payload;

    const synopsis = normalizeText(data?.description ?? data?.synopsis ?? "");
    const rating = data?.user_rate ? String(data.user_rate) : "";

    // Extract categories/genres from taxonomy
    const genres = [];
    const genreTaxonomy = data?.taxonomy?.Genre || data?.genres;
    if (Array.isArray(genreTaxonomy)) {
      genreTaxonomy.forEach((g) => {
    ...
    logger.warn({ err: err.message, mangaId }, "Failed to fetch Secondary metadata");
    return null;
  }
}
...
async function fetchDetailChapters(
  ...
  try {
    ...
    try {
      ...
      const fullInfo = await fetchSecondaryFullMangaInfo(
        apiBase,
        mangaId,
        deadline,
      );
...
function transformChapterResults(
  ...
  return chapterRows
    .map((chapterRow) => {
      ...
      // Use Zod validation helper from shared.js
      return validateChapter(rawObj, logger);
    })
    .filter(Boolean);
}
...
async function processRow(
  ...
  // First-to-Claim: If this chapter is already being handled by another source, skip detail fetch
  // This handles the "race condition" when both Ikiru & Secondary find the same chapter in the same run.
  const chapterHint = row.latest_chapter_number || row.chapter_number;
  if (chapterHint && !(await checkUpdateClaim(redis, title, chapterHint))) {
    logger?.info?.({ title, chapter: chapterHint }, "secondary detail skipped: already claimed by another source");
    return [];
  }
  ...
  if (row?.__directFallback) {
    ...
    try {
      ...
      directRows = await fetchSecondaryRecentChapters(API_BASE, row.manga_id, redis, lookbackHours, deadline);
    } catch (err) {
      logger?.warn?.(
        {
          source: normalized,
    ...
    // If fallback row lacks metadata/title/cover, hydrate it from manga detail endpoint
    // so Discord embeds are not sent as "Unknown Title/Status/Rating" and recent chapters show covers.
    if (
      ...
      ((resolvedRow?.cover_image_url ?? null) === null && (resolvedRow?.cover_portrait_url ?? null) === null)
    ) {
      try {
        const fullInfo = await fetchSecondaryFullMangaInfo(apiBase, row.manga_id, deadline);
        const detail = fullInfo.meta;
  ...
  if (detailState.circuitOpen) {
    const chapterRows = getFallbackChapters(row, now, lookbackHours);
    return transformChapterResults(
      row,
      chapterRows,
      seen,
  ...
  if (!chapterRows) {
    chapterRows = getFallbackChapters(row, now, lookbackHours);
  }
...
async function fetchUpdateList(apiBase, normalized, deadline = 0, lookbackHours = SCRAPER_LOOKBACK_HOURS) {
  const allRows = [];
  const maxPages = 3; // Deep scanning up to 3 pages (approx 120 items)
  const now = Date.now();
  const type = getShngmType(normalized);

  for (let page = 1; page <= maxPages; page++) {
    // Safety check: Don't start a new page fetch if we have less than 5s left
    if (deadline > 0 && Date.now() >= deadline - 5000) {
      break;
    }

    const endpoint = `${apiBase}/v1/manga/list?type=${type}&page=${page}&page_size=40&is_update=true&sort=latest&sort_order=desc`;
    try {
      const res = await fetchWithRetry(
        endpoint,
        JSON_HEADERS,
        FASTCRON_SAFE_CONFIG.REQUEST_TIMEOUT,
        `Secondary update list timeout page ${page}`,
        deadline,
      );
      ...
      // Early Exit: Check if the oldest item on this page is already older than our lookback window.
      // If so, we don't need to check subsequent pages as they will be even older.
      const lastRow = rows[rows.length - 1];
      const lastUpdateRaw = lastRow?.latest_chapter_time ?? lastRow?.updated_at;
      const lastUpdate = parseDateWithFallback(lastUpdateRaw);

      if (lastUpdate && !isWithinHours(lastUpdate, lookbackHours, now)) {
        break;
      }
    } catch (err) {
      // Log and stop pagination on error, but return what we have
      break;
    }
  }
...
export async function scrapeSecondarySourceUpdates(
  ...
  { throwOnError = false, preferredMatcher = null, redis = null, options = {}, deadline = 0 } = {},
  logger = null,
) {
  ...
  try {
    ...
    let rows = [];
    try {
      rows = await fetchUpdateList(API_BASE, normalized, deadline, lookbackHours);
    } catch (feedErr) {
      logger?.warn?.(
...
async function searchPage(apiBase, type, page, keyword, normalized, deadline = 0) {
...
export async function searchShngm(query, source = "shinigami_project", deadline = 0) {
...
function transformRandomResult(row, normalized) {
  ...
  // Note: we can't reliably validate random elements with ChapterScrapeSchema
  // because chapter numbers and urls are missing, but random manga aren't pushed
  // as updates anyway. Returning rawObj directly here.
  return rawObj;
}
