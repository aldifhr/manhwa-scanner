...
export async function scrapeSecondarySourceUpdates(
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const prioritizeDetail = shouldPrioritizeSecondaryEntry({ title, mangaUrl }, preferredMatcher);
...
export async function fetchRandomShinigamiManga(source = "shinigami_project") {
  ...
  try {
    // 1. Fetch page 1 to get total pages from metadata
    const initialEndpoint = `${apiBase}/v1/manga/list?type=${type}&page=1&page_size=40&sort=latest&sort_order=desc`;
    const initialRes = await httpGet(initialEndpoint, {
      headers: {
        Accept: "application/json",
        "User-Agent": HTTP_USER_AGENT,
      },
    ...
    const totalPages = Math.max(1, meta.total_page || 4); // Fallback to 4 if unavailable
    ...
    if (randPage > 1) {
      const endpoint = `${apiBase}/v1/manga/list?type=${type}&page=${randPage}&page_size=40&sort=latest&sort_order=desc`;
