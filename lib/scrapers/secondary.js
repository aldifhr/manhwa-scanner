...
export async function fetchSecondaryRecentChaptersFromMangaDetail(apiBase, mangaId, lookbackHours = FRESH_CHAPTER_HOURS_DEFAULT, deadline = 0) {
...
export async function fetchSecondaryMangaDetail(apiBase, mangaId, deadline = 0) {
  const endpoint = `${apiBase}/v1/manga/detail/${mangaId}`;
  const res = await fetchWithRetry(
    endpoint,
    JSON_HEADERS,
...
async function processRow(
  ...
  if (row?.__directFallback) {
    ...
    if (
      ...
      try {
        const detail = await fetchSecondaryMangaDetail(apiBase, row.manga_id, deadline);
