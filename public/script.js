...
async function renderSuccessChart(data) {
  ...
  trendChart = new Chart(ctx, {
    ...
    data: {
      labels: data.labels || [],
      datasets: [
        { label: "Sent ✅",   data: data.sent   || [], backgroundColor: "#10B981", borderRadius: 4, borderSkipped: false },
        { label: "Failed ❌", data: data.failed || [], backgroundColor: "#EF4444", borderRadius: 4, borderSkipped: false },
      ],
    },
    options: {
      responsive: true,
...
function calculateUptime(logs, hours) {
  ...
  return Math.round((recent.filter((l) => l.tag === "sent").length / recent.length) * 100);
}

function getTopManhwa(logs) {
  const counter = {};
  logs?.filter((l) => l.tag === "sent" && l.message?.includes("Chapter")).forEach((l) => {
    const title = l.message.split(" — ")[0]?.trim() || l.message.split("Chapter")[0]?.trim();
    if (title) counter[title] = (counter[title] || 0) + 1;
  });
  return Object.entries(counter).sort(([, a], [, b]) => b - a).slice(0, 5).map(([title, count]) => ({ title, count }));
}

// ===== AUTH =====
function checkAuth() {
  if (!secret) { $("modalOverlay").classList.add("show"); return false; }
  return true;
}
...
function renderStatsExtended(statusData, uptimeData) {
  ...
  $("statUptime24h").className   = `stat-value ${uptimeData?.uptime24h >= 95 ? "green" : uptimeData?.uptime24h >= 80 ? "amber" : "red"}`;
  $("statUptime7d").textContent  = uptimeData?.uptime7d  ?? "—";
  $("statUptime7d").className    = `stat-value ${uptimeData?.uptime7d  >= 95 ? "green" : uptimeData?.uptime7d  >= 80 ? "amber" : "red"}`;
...
function renderLogs(data) {
  ...
  if (!logs.length) { list.innerHTML = `<li class="empty">Belum ada log</li>`; return; }
...
async function addManga() {
  ...
  try {
    ...
    if (!r.ok) { showAlert(data.error || "Gagal menambah manga"); return; }
    titleInput.value = "";
    urlInput.value   = "";
...
async function deleteManga(title) {
  ...
  try {
    ...
    const data = await r.json();
    if (!r.ok) { showAlert(data.error || "Gagal menghapus"); return; }
    renderWhitelist(data);
  } catch (e) {
    showAlert("Gagal: " + e.message);
  }
...
async function loadAll() {
  ...
  if (trendR.status === "fulfilled" && trendR.value.ok) {
    try { renderSuccessChart(await trendR.value.json()); } catch (e) { console.error("Chart error:", e); }
  }
...
window.addEventListener("focus", () => { if (secret) loadAll(); });
