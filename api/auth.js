...
import { safeParse } from "./incidents.js";
...
async function readRawBody(req) {
  ...
  if (!result && contentType.includes("application/json")) {
    logger.warn(
      "Raw body empty despite application/json Content-Type (possibly consumed by middleware)",
    );
  }
...
async function handleLogin(req, res) {
  if (!isDashboardPasswordConfigured()) {
    return res
      ...
      .json(
        createErrorResponse(
          "SERVER_ERROR",
          "DASHBOARD_PASSWORD belum diset di server",
        ),
      );
  }
  ...
  if (throttle.limited) {
    ...
    return res
      ...
      .json(
        createErrorResponse(
          "RATE_LIMITED",
          `Terlalu banyak percobaan. Coba lagi dalam ${throttle.retryAfterSec} detik.`,
        ),
      );
  }
  ...
  if (!password) {
    return res
      .status(400)
      .json(
        createErrorResponse("INVALID_INPUT", "Password tidak boleh kosong"),
      );
  }
    ...
    if (failed.limited) {
      ...
      return res
        ...
        .json(
          createErrorResponse(
            "RATE_LIMITED",
            `Terlalu banyak percobaan. Coba lagi dalam ${failed.retryAfterSec} detik.`,
          ),
        );
    }
    return res
      .status(401)
      .json(createErrorResponse("UNAUTHORIZED", "Password salah"));
  }
  ...
  if (!token)
    return res
      .status(500)
      .json(createErrorResponse("SERVER_ERROR", "Session secret belum diset"));
...
function handleLogout(req, res) {
  ...
  return res
    .status(200)
    .json(createSuccessResponse({ ok: true, wasAuthenticated }));
}
...
function handleStatus(req, res) {
  ...
  return res.status(200).json(
    createSuccessResponse({
      authenticated: isDashboardSessionAuthorized(req),
    }),
  );
}
...
export default async function handler(req, res) {
  ...
  if (!expectedMethod) {
    return res
      .status(400)
      .json(createErrorResponse("INVALID_ACTION", "Unknown action"));
  }
