...
async function runAdvancedMigration() {
  console.log(
    "🚀 Starting Advanced Redis Migration & Feature Initialization...",
  );
  ...
  if (rawLegacy && Array.isArray(rawLegacy)) {
    console.log(
      `Found legacy whitelist with ${rawLegacy.length} items. Migrating...`,
    );
  ...
  do {
    ...
    if (keys.length > 0) {
      const p = keys.map(async (key) => {
        ...
        if (subsCount > 0) {
          await redis.zadd("manga:popularity_index", {
            score: subsCount,
            member: titleKey,
          });
          totalSubs += subsCount;
  ...
  console.log(
    `✅ Initialized popularity index for ${count} mangas (total followers: ${totalSubs}).`,
  );
