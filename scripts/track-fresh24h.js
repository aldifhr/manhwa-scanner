...
function isBlockingClaim(claim, nowMs = Date.now()) {
...
function isWithin24h(updatedTime) {
  const ts = new Date(updatedTime || "").getTime();
  if (!Number.isFinite(ts)) return false;
  return Date.now() - ts <= 24 * 60 * 60 * 1000;
}

function toArrayHmget(keys, result) {
...
async function main() {
  ...
  const duplicateKeys = [...new Set(meta.map((entry) => entry.duplicateKey).filter(Boolean))];
